...
|--------|-----------------|----------|-----------------|
| **Archon** | Claude Code companion daemon; full-document RAG over local file collections | Python / asyncio | LanceDB + fastembed (custom pipeline) |
...
| **Notable design weakness** | `archon_toolkit_search.py` is 884 lines | No TypeScript; no dependency injection; console.log in production code | Python `< 3.12` hard upper bound; sync route handlers block uvicorn event loop | `theflow` is undocumented outside Kotaemon; `print()` in production code | No health check API; `user_id + agent_id` AND filter silently returns empty | `AgentFactory` has dead commented-out XML code; `FIXME` in agent base |
...
| **PDF (text)** | ✅ trafilatura/docling | ✅ pdf-parse + Tesseract fallback | ✅ pdfminer | ✅ pypdf / OCR / multimodal | ❌ (no doc ingest) | ✅ 38 formats |
...
| **Metadata filtering** | ❌ | ❌ | ✅ filter by `doc_id` list (`ContextFilter`) | ✅ by `file_id` in LanceDB | ✅ structured `filters` dict with AND/OR operators over scoping fields | ✅ full Postgres-style predicate filters on `metadata` JSONB |
...
| **default `top_k` retrieved** | 5 (configurable `top_k_retrieve`) | 4 per workspace | 2 (critically low default) | `top_k * 10` candidates pre-reranking | configurable; `threshold=0.1` in v3 | configurable; `limit` in `SearchSettings` |
...
| **Default reranker model** | `BAAI/bge-reranker-v2-m3` (cross-encoder, ONNX) | `Xenova/ms-marco-MiniLM-L-6-v2` (ONNX, CPU-only) | `cross-encoder/ms-marco-MiniLM-L-2-v2` | `cohere/rerank-v4.0-fast` (default when Cohere key present) | `cross-encoder/ms-marco-MiniLM-L-6-v2` (SentenceTransformer option) | `mixedbread-ai/mxbai-rerank-large-v1` (via TEI) |
...
| **Embedding model validation on startup** | ✅ `install.py validate_providers()` tests stack before committing config | ❌ | ❌ | ❌ | ❌ | ❌ |
...
| **Multi-tenancy at storage level** | ❌ All collections share one LanceDB | ❌ Workspaces share one SQLite | ❌ Single collection per deployment | ⚠️ `user_id`-scoped document tables | ✅ `user_id`/`agent_id`/`app_id`/`run_id` scoping on every query | ✅ PostgreSQL schema-per-project; `x-project-name` header routing |
...
| **MCP server (exposes tools)** | ✅ FastMCP HTTP server: `search`, `search_with_context`, `ingest_file`, `ingest_directory`, `list_collections`, `get_collections_meta`, `get_collection_meta`, `list_documents`, `delete_document` (9 tools); ArchonToolkit: 10 session-level search tools | ❌ Does not expose itself as MCP server | ❌ | ❌ | ✅ Hosted MCP at `https://mcp.mem0.ai/mcp`; self-hosted `mem0-mcp` package | ❌ |
| **MCP client (consumes tools)** | ✅ Full MCP client via ArchonMCPServer | ✅ `MCPCompatibilityLayer`; any MCP server auto-discovered and exposed as agent plugin | ❌ | ✅ `MCPManager`; MCP servers configured via Settings UI and used in ReAct/ReWoo agents | ❌ | ❌ |
...
| **CLI** | ✅ `archon` entry point; 8 search subcommands; `collection add/remove/reindex` | ❌ Scripts only; no installed CLI | ❌ Scripts only | ⚠️ `kotaemon` CLI via trogon TUI; limited | ✅ Node + Python CLI: `add`, `search`, `get`, `delete` | ✅ `r2r` CLI: `serve`, `db upgrade`, `generate-private-key` |
...
| **Telegram** | ✅ (core feature — Archon is a Telegram bot) | ✅ optional `node-telegram-bot-api` | ❌ | ❌ | ❌ | ❌ |
| **Health endpoint** | ✅ `GET /health` on FastMCP HTTP server | ✅ `GET /api/ping → { online: true }` (no deep checks) | ✅ `GET /health → { status: "ok" }` (no component checks) | ❌ None | ❌ None | ✅ `GET /v3/health`; Docker healthcheck interval 6s; `/v3/system/status` for authorized users |
...
| **Health / diagnostics** | ✅ `archon doctor` CLI: collection staleness, model mismatch, indexing state, service status; `GET /health` HTTP | ⚠️ `GET /api/ping` (no deep checks); admin panel stats | ⚠️ `GET /health → ok` (no component liveness checks) | ❌ No health endpoint or doctor command | ❌ No health check API | ✅ `GET /v3/health`; `/v3/system/status`; Sentry; Fluent-Bit log shipping config |
| **Monitoring** | ✅ `IndexingNotificationMonitor` — Telegram summary when all collections reach terminal state | ❌ No metrics; PostHog telemetry opt-out | ❌ LlamaIndex event callbacks to stdout; no metrics endpoint | ❌ No structured monitoring | ❌ Python `logging` only; no metrics | ✅ Sentry SDK; Fluent-Bit; `request_log` table for rate-limit audit; no Prometheus endpoint |
...
| **Scheduled maintenance** | ✅ `JobScheduler` for user-defined tasks; `cleanup-orphan-documents` equivalent in watcher | ✅ Bree: cleanup every 12h/8h; doc sync every 1h | ❌ | ❌ | ❌ | ✅ APScheduler `VACUUM` / `VACUUM ANALYZE` daily at 3am; `vacuum_schedule` configurable |
| **Auto-update** | ✅ `archon update [--tag <version>]` | ❌ | ❌ | ❌ | ❌ | ✅ `r2r db upgrade` (Alembic migrations) |
...
| **Notable test modules (Archon)** | `test_sync.py` 4,613 lines · `test_install.py` 1,892 · `test_server.py` 1,611 · `test_pipeline.py` 1,172 · `test_store.py` 1,024 | — | — | — | — | — |
...
| Attribute | Archon | AnythingLLM | PrivateGPT | Kotaemon | mem0 | R2R |
|-----------|--------|-------------|------------|----------|------|-----|
| **Vector search complexity** | Sub-linear — LanceDB IVF-PQ ANN | Sub-linear — LanceDB auto-index (default); varies by provider | Delegated to backend; Qdrant HNSW O(log n); Chroma brute-force for small collections | ChromaDB brute-force for small; HNSW at scale; LanceDB IVF-PQ; Milvus IVF/HNSW | O(log n) HNSW via Qdrant (default); delegated to backend | pgvector HNSW O(log n) or IVFFlat O(√n); binary quantization 32x storage reduction |
| **Reranker throughput** | ONNX CPU; batch inference; practical for interactive use | ~5.2s/20 docs on i7 (ONNX CPU) — prohibitively slow for interactive production use | fast (MiniLM-L-2); practical | Cohere API (fast); LLM reranker via ThreadPoolExecutor (variable, API-bound) | Fast if Cohere; slower if SentenceTransformer or LLM reranker | External TEI service; network hop per rerank request |
...
| **Archon doctor diagnostics** | ✅ Collection staleness, model mismatch, indexing state, service status | ❌ | ❌ | ❌ | ❌ | ✅ `/v3/system/status` + Docker healthcheck |
| **Indexing completion notification** | ✅ Telegram summary when all collections reach terminal state | ❌ | ❌ | ❌ | ✅ Webhooks: `memory_add/update/delete/categorize` | ❌ |
...
| Attribute | Archon | AnythingLLM | PrivateGPT | Kotaemon | mem0 | R2R |
|-----------|--------|-------------|------------|----------|------|-----|
| **Native LLM session integration** | ✅ MCP tools injected directly into Claude Code sessions; search context provided at session startup via `ContextProvider` | ✅ AIbitat agent framework; `@agent` invocation; automatic mode with native tool calling | ❌ External clients use the REST API | ✅ ReAct/ReWoo reasoning pipelines; MCP tool consumption | ✅ `@mem0/vercel-ai-provider` wraps any AI model transparently; `user_id` as model option | ✅ RAG Agent multi-turn with memory; Research Agent with o3-mini planning |
| **Conversation history as retrieval signal** | ✅ `HistoryManager` session history; `ContextProvider` injects history context at startup | ⚠️ `fillSourceWindow` backfills sources from past 20 messages heuristically | ❌ | ✅ `AddQueryContextPipeline` uses last N turns to reformulate query | ✅ Full history store (SQLite); v3 retrieves top-10 related memories before each `add()` | ✅ Conversation sessions stored; multi-turn RAG agent |
| **Reminder / drift prevention** | ✅ `ContextReminder` injects `REMINDER.md` every N messages/tokens to prevent context drift | ❌ | ❌ | ❌ | ❌ | ❌ |
...
| # | Gap | Best example | What to build |
|---|-----|-------------|---------------|
| 6 | **REST API alongside MCP** | R2R `/v3/`, AnythingLLM `/api/v1/` | Add a thin FastAPI REST endpoint layer on the FastMCP server for non-Claude consumers. Enables curl / Python scripts / third-party tools to query search without an MCP client. |
...
| 13 | **Multi-tenancy** | R2R (schema-per-project), mem0 (4-level scoping). Archon is single-user by design (Telegram whitelist). If Archon ever moves to multi-user, the storage model would need a fundamental redesign. |
