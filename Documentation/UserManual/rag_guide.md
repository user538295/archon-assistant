...
5. Detects NVIDIA GPU via `nvidia-smi`; installs `fastembed-gpu` automatically if found
...
| `providers` | list[str] | `[]` | ONNX execution providers (`[]` = CPU; `["CUDAExecutionProvider"]` = GPU) |
...
**GPU acceleration** (set automatically by `archon rag install` if NVIDIA GPU detected):
...
- **GPU support: NVIDIA only** — the installer detects NVIDIA via `nvidia-smi`. Apple Silicon GPU and AMD/ROCm are not supported; those systems use CPU.
