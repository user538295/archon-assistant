...
| Search-watcher | Search | Add incremental re-indexing via inotify (Linux) / FSEvents (macOS) watcher so history files are indexed automatically without manual `archon search ingest`. | Low — requires platform watcher integration | M | `archon/search/` |
| Search-windows | Distribution | `archon search start/stop` currently stubs out on Windows with a message directing users to run `python -m archon.search.server` manually. Implement Windows service registration via `pywin32` or NSSM. | Low — Windows-only gap | M | `archon/search/install.py`, `archon/platform/windows/` |
