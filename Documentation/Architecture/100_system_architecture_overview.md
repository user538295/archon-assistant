...
graph TB
    subgraph Daemon["Archon Daemon — single asyncio event loop"]
        ...
        subgraph AILayer["ai/ — AI and background execution layer"]
            ...
            EM["EventMapper<br/>Maps SDK messages to typed<br/>event dataclasses (16 types)"]
...
graph TB
    subgraph ai["archon/ai/"]
        ...
        em["event_mapper.py<br/>EventMapper<br/>map_messages(stream) → events<br/>ThinkingResult · ToolStarted · ToolResult<br/>Response · ErrorEvent · ClassificationEvent<br/>SubagentStarted · SubagentStopped<br/>PlanEvent · PromotionEvent · RoutingEvent<br/>FallbackNoticeEvent · RecoveryEvent<br/>WaveStarted · WaveCompleted · ReminderInjectedEvent"]
