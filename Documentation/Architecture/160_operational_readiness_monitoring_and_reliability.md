...
|---|---|
| `INFO` (default) | Daemon start/stop, session created/stopped, message received `(N chars)`, model set, skill queued for next message, Search server URL |
| `DEBUG` | Context queued for next message |
| `WARNING` | Unauthorized user dropped, typing indicator failed, Telegram delivery failed, session cleanup timed out, `config.toml` restored from backup, Search server unreachable |
...
The `health_check.sh` script reports process status, Search server status, the last 5 log errors, disk usage, and memory:
