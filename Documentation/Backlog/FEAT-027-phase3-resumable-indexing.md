...
- `RagCollectionSync.sync()` — load `processed_paths` on sync start; pass to `ingest_directory` as `exclude_paths`; accumulate newly processed paths and flush to state at every 50-file batch boundary and on completion; resume collections that are in `existing & desired` with status `PENDING` or `FAILED` (not just newly added collections); update `_reset_stale_in_progress()` to preserve `processed_paths`
- CLI `_run_collection_reindex` — clear `processed_paths` (and full collection state) from state file before running the re-index
- MCP `_handle_rag_collection_reindex` — clear `processed_paths` (and full collection state) from state file before running the re-index
...
- `archon rag status` display changes — progress display already handles the resume offset via existing `processed_files`/`total_files` fields
...
- `archon rag status` display logic — `processed_files`/`total_files` already drive the display
...
- **FAILED collections auto-resumed**: collections with `FAILED` status are auto-resumed on the next `sync()` (Step 6.5). There is no backoff or retry limit — a collection that fails due to a persistent error (corrupt file, permission denied) will be retried on every startup. Users should run `archon rag reindex <collection>` if a collection fails repeatedly. Phase 4 may add retry limiting.
...
**`archon/rag/pipeline.py`** — `RagPipeline.ingest_directory`
```python
...
**`archon/rag/sync.py`** — `RagCollectionSync.sync()` (inner ingestion loop)
...
**`archon/cli/rag_cmd.py`** — `_run_collection_reindex`
- Before calling `pipeline.ingest_directory`, instantiate `IndexingStateStore(Path(cfg.rag.db_path))` and call `state_store.remove_collection(col_name)` to wipe all prior state
...
**`archon/ai/archon_toolkit_rag.py`** — `_handle_rag_collection_reindex`
- After resolving `col_name` and before ingesting, instantiate `IndexingStateStore(Path(cfg.rag.db_path))` and call `remove_collection(col_name)`
...
- **`test_reset_stale_preserves_processed_paths`** (unit, `tests/rag/test_sync.py`): IN_PROGRESS state with `processed_paths=["a","b"]`; after `_reset_stale_in_progress()`, status=PENDING and `processed_paths=["a","b"]`
...
- **`test_handle_rag_collection_reindex_clears_state`** (unit): MCP tool clears state before ingesting
...
- **Tests (TDD)** — `tests/rag/test_pipeline.py` (or existing pipeline test file):
...
- **Description**:
  ...
  - Wrap the call in a `try/except` — state clear failure is non-fatal (log warning; proceed with reindex)
  - Import `IndexingStateStore` from `archon.rag.progress`
- **Releasable**: after this task, `archon rag reindex` guarantees a clean full re-index with no stale resume state
...
- **Description**:
  ...
  - Non-fatal: log warning on failure; continue with ingest
- **Releasable**: after this task, `rag_collection_reindex` MCP tool guarantees a clean full re-index with no stale resume state
- **Tests (TDD)** — `tests/ai/test_archon_toolkit_rag.py`:
  - Unit: `test_handle_rag_collection_reindex_clears_state` — mock `IndexingStateStore`; assert `remove_collection(col_name)` called before ingest
  - Unit: `test_handle_rag_collection_reindex_state_clear_failure_non_fatal` — `remove_collection` raises; ingest proceeds normally
  - Checkpoint: `uv run pytest tests/ai/test_archon_toolkit_rag.py -v --no-cov -k "reindex"`
