...
## Acceptance criteria
- [ ] Fresh install produces a `config.toml` that is a near-identical copy of `config.toml.example` with `allowed_user_ids` and `working_directory` lines replaced by real values via regex
- [ ] `_default_config()` function is deleted from `install.py`
- [ ] `_SPARSE_PATHS` contains `"examples"` and not a stale root-level `"config.toml.example"` entry
...
- [ ] New tests cover the template rendering and sparse-path fix
...
- [ ] `examples/config.toml.example` has `default = "claude-sonnet-4-6"` uncommented in `[models]`
