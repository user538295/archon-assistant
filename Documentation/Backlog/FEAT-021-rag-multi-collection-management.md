# FEAT-021 — Search Declarative Multi-Collection Management
**Purpose**: Let operators declare a list of directory paths in `config.toml [search] collections` and have Archon automatically keep LanceDB in sync — ingesting added paths, dropping removed ones.
...
The Search pipeline already supports arbitrary named collections — `SearchPipeline.ingest_directory(path, collection)` creates a LanceDB table on first use. However, there is no config-driven way to declare which directories to index: operators must run `archon search ingest` manually, stop/start the service themselves, and there is no mechanism to remove a collection when a path is no longer wanted.
...
### In Scope
- New `collections: list[str]` field in `SearchConfig`, default `["~/.archon/history/sessions", "~/.archon/workspace"]`
- `path_to_collection_name(path: str) -> str` — deterministic, collision-safe name derivation
- `SearchCollectionSync` class — reconciles config list vs LanceDB state; produces `SyncResult`
- `SearchStore.drop_collection(name: str)` — drops a LanceDB table
- `SearchStore.rename_collection(old: str, new: str)` — renames a LanceDB table (used by migration step in `sync()`)
- `server.py:main()` calls sync before accepting connections
- `archon search sync` CLI command — manual full reconciliation
- `archon search collection list` — shows all collections with source path, doc count, chunk count, and indexed status
- `archon search collection add <path>` — appends path to config, immediately ingests
- `archon search collection remove <path>` — removes path from config, immediately drops collection if service is stopped (warns otherwise)
- `archon search help` — print `archon search` subcommand help
- `archon search collection help` — print `archon search collection` subcommand help
- `history_collection` derived automatically from the history-sessions path instead of being user-editable
- Updated `search_guide.md` with declarative collections and CLI management sections
- `archon search install` bootstraps the default collections (replaces `create_history_collection`)

### Out of Scope
- File-watch based live re-indexing (sync runs at startup and on explicit command only)
- Per-collection configuration (chunk size, model) — all share `[search]` config
- User-facing collection renaming CLI (e.g., `archon search collection rename`) — internal rename via `SearchStore.rename_collection()` is used only for the `archon-history` → `sessions` migration
- Incremental re-index (changed files) — sync always re-ingests the full directory
- Access control per collection
- `archon search ingest` with a manual `--collection` flag (still works unchanged for one-off use)

---

## Acceptance criteria
- [x] `SearchConfig` has `collections: list[str]` with default `["~/.archon/history/sessions", "~/.archon/workspace"]`
- [x] `path_to_collection_name` converts any path to a valid, unique LanceDB table name
- [x] `SearchCollectionSync.sync()` ingests paths in config not yet in LanceDB
- [x] `SearchCollectionSync.sync()` drops LanceDB collections whose paths are no longer in config
- [x] `server.py:main()` runs sync before the HTTP server starts accepting connections
- [x] `archon search sync` runs sync and prints added/removed/unchanged counts
- [x] `archon search collection list` shows each collection with source path, doc/chunk counts, and indexed/orphan(managed)/unmanaged status
- [x] `archon search collection add <path>` appends path to `config.toml [search] collections`, immediately ingests, prints confirmation
- [x] `archon search collection add <path>` on a path already in config prints "already registered" and exits 0
- [x] `archon search collection remove <path>` removes path from config, drops collection if service is stopped; warns and requires `--force` if service is running
- [x] `archon search collection remove <path>` on a path not in config prints error and exits 1
- [x] `archon search help` prints `archon search` subcommand listing and exits 0
- [x] `archon search collection help` prints collection subcommand listing and exits 0
...
- [x] If `config.toml` contains `[search] history_collection`, a WARNING is logged and a Telegram notification is sent at startup; the key is ignored
- [x] Removing a path from `collections` and restarting the service (or running `archon search sync`) drops its LanceDB table
- [x] Adding a path to `collections` and running `archon search sync` ingests the directory
...
- [x] Sync does not drop collections not managed by sync (e.g., created via `archon search ingest --collection` flag)
- [x] `SyncResult` includes a `skipped` field listing unmanaged collections that were not touched
- [x] `archon search collection add <path>` prints a warning if the Search service is running
...
- [x] `sync_timeout_seconds` (default 30) is configurable in `[search]` config
...
- `SearchPipeline.ingest_directory()` and `SearchPipeline.search()` — collection logic unchanged
- `server.py` MCP tools (`search`, `ingest_file`, etc.) — accept `collection` parameter unchanged
- `archon search ingest` CLI command — still works for manual one-off ingests
...
- `archon search sync` must be run (or service restarted) for config changes to take effect. There is no file-watch mechanism.
...
- Sync only removes collections that it previously created (tracked via a manifest file). Collections created via `archon search ingest --collection` are never dropped by sync. This is by design.
...
**Migration algorithm** (executed once, at the start of `SearchCollectionSync.sync()`, before any add/remove logic):
...
**Config migration** (at `load_config()` time): if the loaded TOML contains a `history_collection` key under `[search]`, log a WARNING and emit a Telegram notification: `"config.toml [search] history_collection is no longer supported and is being ignored. Remove this key to silence this warning."` The key is silently discarded; the derived name is used instead.

**Manual migration fallback** (for operators who cannot upgrade in place): run `archon search sync` once after upgrading. The migration detection in `sync()` will rename the table automatically. No data is lost.
...
New module-level function in `archon/search/sync.py`.
...
**This function is collision-unaware by design.** It always returns the same name for a given path. Collision resolution is applied in `SearchCollectionSync.sync()`, which tracks a global `{name: path}` mapping across all configured paths and modifies derived names only when conflicts are detected.
...
### `SearchConfig` changes in `archon/config/loader.py`
...
`history_collection` is removed. Callers that needed `cfg.search.history_collection` (gateway, server, context_provider) will call `path_to_collection_name(cfg.history.directory + "/sessions")` instead. Since `SearchConfig` does not know about `HistoryConfig`, this derivation happens at the call sites in `gateway.py` and `server.py`.

**`SearchPipeline` constructor change**: the existing `SearchPipeline.__init__` and `create_pipeline` factory both accept `history_collection` as a parameter (read from `cfg.search.history_collection`). Removing `history_collection` from `SearchConfig` will cause `AttributeError` at all call sites that use `cfg.search.history_collection`. As part of Task 2.2, `SearchPipeline.__init__` and `create_pipeline` must have the `history_collection` parameter removed; all call sites in `server.py` and `gateway.py` must derive the history collection name via `path_to_collection_name(...)` instead.

### `SearchStore.drop_collection(name: str) -> None` in `archon/search/store.py`

`SearchStore` uses `lancedb.connect_async()` which returns an `AsyncConnection`. Its `list_tables()` and `drop_table()` are native async methods — no `asyncio.to_thread()` wrapper needed.
...
### `SearchStore.rename_collection(old: str, new: str) -> None` in `archon/search/store.py`
...
async def rename_collection(self, old: str, new: str) -> None:
    ...
    try:
        ...
        # re-ingest is handled at the caller (SearchCollectionSync migration step)
        raise NotImplementedError("rename_table not available; use copy-ingest + drop")
```

This method encapsulates the LanceDB rename operation so that `SearchCollectionSync` (which holds a `SearchPipeline`, not a raw db connection) can call it without violating encapsulation.
...
### `SyncResult` and `SearchCollectionSync` in `archon/search/sync.py` (new file)
...
**Manifest file**: `SearchCollectionSync` maintains a manifest at `{db_path}/sync_manifest.json` (where `db_path` is `Path(cfg.search.db_path).expanduser()`) recording `{collection_name: source_path}` for every sync-managed collection. The manifest is always co-located with the LanceDB data directory so that moving `db_path` keeps the manifest and database in sync. On each sync:
- Collections that appear in the manifest but are no longer in config are dropped.
- Collections not in the manifest (created manually via `archon search ingest --collection`) are never dropped by sync. They appear in `SyncResult.skipped`.
...
sync = SearchCollectionSync(pipeline)
sync_timeout = cfg.search.sync_timeout_seconds  # default: 30
if sync_timeout == 0:
    # Run sync entirely in background (don't wait at all)
    asyncio.create_task(sync.sync(cfg.search.collections))
    logger.info("Search sync: running in background (sync_timeout_seconds=0)")
else:
    try:
        result = await asyncio.wait_for(
            sync.sync(cfg.search.collections),
            timeout=sync_timeout,
        )
        logger.info("Search sync: added=%s removed=%s unchanged=%s errors=%s skipped=%s",
                    result.added, result.removed, result.unchanged, result.errors, result.skipped)
    except asyncio.TimeoutError:
        logger.warning("Search sync timed out after %ds; restarting sync in background", sync_timeout)
        asyncio.create_task(sync.sync(cfg.search.collections))
```

**Timeout configuration**: add `sync_timeout_seconds: int = 30` to `SearchConfig`. Setting `sync_timeout_seconds = 0` skips `asyncio.wait_for` entirely and creates a background task immediately — this avoids the partial-sync race that would occur if `asyncio.wait_for(coro, timeout=0)` were used (which would immediately cancel the coroutine and raise `TimeoutError`, resulting in a double-sync). During background ingest after a timeout or `0` setting, searches may return partial results — this is documented behaviour.
...
In `gateway.py` (where `search_url` is built) and `server.py` (where `create_app` is called), replace `cfg.search.history_collection` with:
...
### CLI: `archon search sync`

New action in `archon/cli/search_cmd.py`:
...
def _run_sync(args: Namespace) -> int:
    ...
    result = asyncio.run(_do_sync(pipeline, cfg.search.collections))
...
### CLI: `archon search collection list`

```python
def _run_collection_list(args: Namespace) -> int:
    # Open store, list_collections(), cross-reference with cfg.search.collections and the manifest
    # Three states:
    #   indexed        — in config AND in LanceDB (managed by sync)
    #   orphan (managed) — in manifest, NOT in config; will be removed on next sync
    #   unmanaged      — in LanceDB but NOT in manifest (created via `archon search ingest --collection`); will NOT be touched by sync
    # Print: name | path | docs=N | chunks=M | status
```

### CLI: `archon search collection add <path>`

```python
def _run_collection_add(args: Namespace) -> int:
    path = str(Path(args.path).expanduser().resolve())
    cfg = load_config()
    if path in [str(Path(p).expanduser().resolve()) for p in cfg.search.collections]:
        print(f"Already registered: {args.path}")
        return 0
    svc_running = get_search_service().status().running
    if svc_running:
        print("Warning: Search service is running. The collection will be indexed "
              "but opening a second LanceDB connection may cause write conflicts. "
              "Consider stopping the service first: archon search stop")
    # Write to config.toml using tomlkit (same pattern as configure_providers)
    _config_collections_append(config_path, args.path)
    pipeline = create_pipeline(load_config().search)
    # Derive collection name: use manifest if path was previously synced, else naive derivation
    # The name may be updated (collision-resolved) on the next archon search sync
...
**NOTE — collision-aware name handling for `collection add`**: The collection name used for immediate ingest is derived via `path_to_collection_name(path)` (collision-unaware). If a collision exists with an already-registered collection, the immediately-ingested collection will use the naive name. The collision will be detected and resolved on the next sync, which may rename or re-ingest the collection. If the manifest already contains an entry for the same path (e.g., from a previous sync), use the existing name from the manifest; otherwise derive via `path_to_collection_name` (the name will be collision-checked during the next sync). For correctness, users with collision-prone path names should prefer editing `config.toml` directly and running `archon search sync` instead of using `collection add`.

### CLI: `archon search collection remove <path>`

```python
def _run_collection_remove(args: Namespace) -> int:
    path = str(Path(args.path).expanduser().resolve())
    cfg = load_config()
    normalized = [str(Path(p).expanduser().resolve()) for p in cfg.search.collections]
    if path not in normalized:
        print(f"Error: path not in collections: {args.path}")
        return 1
    svc_running = get_search_service().status().running
    if svc_running and not args.force:
        print("Error: Search service is running. Stop it first or use --force.")
        print("  archon search stop && archon search start")
        return 1
    if svc_running:
...
`_manifest_lookup_by_path(manifest_path: Path, resolved_path: str) -> str | None` reads the manifest JSON (`{collection_name: source_path}`) and returns the collection name whose value matches `resolved_path`, or `None` if no entry is found. (defined in `archon/search/sync.py` alongside other manifest utilities)
...
Private helpers in `archon/cli/search_cmd.py`. These use tomlkit's native `Array` operations to preserve inline comments and multi-line formatting (per ADR-08). They also follow the atomic write pattern from `config_rw.py` (temp file + `os.replace()`) to prevent partial writes.

```python
def _config_collections_append(config_path: Path, path: str) -> None:
    doc = tomlkit.parse(config_path.read_text())
    search_section = doc.setdefault("search", tomlkit.table())
    if "collections" not in search_section:
        search_section["collections"] = tomlkit.array()
    search_section["collections"].append(path)  # native Array.append — preserves existing formatting
...
def _config_collections_remove(config_path: Path, path: str) -> None:
    ...
    if "collections" in search_section:
        new_cols = tomlkit.array()
        for p in search_section["collections"]:
            if str(Path(p).expanduser().resolve()) != resolved:
                new_cols.append(p)
        search_section["collections"] = new_cols
    tmp = config_path.with_suffix(".tmp")
    tmp.write_text(tomlkit.dumps(doc))
    os.replace(tmp, config_path)
```
...
### `archon search help` and `archon search collection help`

In `main.py`, `p_search` and `p_collection` parser references are captured and passed down via `args` or via a closure. The `help` subcommand under each group calls `parser.print_help()` on the relevant parser.

```python
# main.py
search_sub.add_parser("help", help="Show search subcommand help")
collection_sub.add_parser("help", help="Show collection subcommand help")

# search_cmd.py dispatch
"help": lambda args: (p_search.print_help(), 0)[1],
# collection dispatch
"help": lambda args: (p_collection.print_help(), 0)[1],
```

Since `print_help()` needs the parser object, `run_search()` receives it as a parameter, or the parser is recreated from `main.py` refs. The cleanest pattern: pass `search_parser` and `collection_parser` into `run_search(args, search_parser, collection_parser)`.
...
search_sub.add_parser("help", help="Show this help")
# ... existing: install, uninstall, start, stop, status, ingest, sync ...
search_sub.add_parser("sync", help="Reconcile collections with config")

p_collection = search_sub.add_parser("collection", help="Manage indexed collections")
...
`SearchInstaller.create_history_collection()` → replaced by calling `SearchCollectionSync.sync(cfg.search.collections)`. This ingests all default collections (history + workspace) on first install.
...
- **`test_search_config_default_collections`** (unit): default contains history and workspace paths
...
- **`test_config_collections_append_writes_tomlkit`** (unit): path appended to `[search] collections` in config file
...
- **`test_search_help_prints_usage`** (unit): `archon search help` → prints search subcommand list, exit 0
- **`test_collection_help_prints_usage`** (unit): `archon search collection help` → prints collection subcommand list, exit 0
- **`test_sync_integration`** (integration): temp LanceDB + real SearchCollectionSync; add/remove paths → verify state
...
- [x] `search_guide.md`, sections: Declarative Collections + CLI Collection Management, path: `Documentation/UserManual/search_guide.md`
- [x] `examples/config.toml.example` — add `collections` key with defaults and comments
- [x] `Documentation/Architecture/180_search_architecture.md` — update collection management section
...
- [x] **File**: `archon/search/sync.py` (new file)
- **Depends on**: nothing
- **Description**:
  ...
  - No collision handling here; collision handling is in `SearchCollectionSync.sync()`
...
- **Tests (TDD)** — `tests/search/test_sync.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_sync.py -k "path_to_collection_name" -v`

#### Task 1.2 — Add `SearchStore.drop_collection()`
- [x] **File**: `archon/search/store.py`
- **Depends on**: nothing
- **Description**:
  - `async def drop_collection(self, name: str) -> None`
  - `SearchStore` uses `lancedb.connect_async()` → `AsyncConnection`; use native async methods (no `asyncio.to_thread` needed)
  - `names = (await self._db.list_tables()).tables` — `list_tables()` returns `ListTablesResponse` with `.tables: list[str]`
...
- **Tests (TDD)** — `tests/search/test_store.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_store.py -k "drop_collection" -v`

#### Task 1.3 — Add `SyncResult` dataclass and `SearchCollectionSync` class
...
- **Description**:
  ...
  - `class SearchCollectionSync: __init__(self, pipeline: SearchPipeline) -> None`
  - `async def sync(self, collections: list[str]) -> SyncResult`:
    ...
    3. Load manifest from `{db_path}/sync_manifest.json` (i.e. `Path(cfg.search.db_path).expanduser() / "sync_manifest.json"`) → `managed_names: set[str]`
  ...
  - Manifest file: `<db_path>/sync_manifest.json` (i.e. `Path(cfg.search.db_path).expanduser() / "sync_manifest.json"`), format `{collection_name: source_path}`
...
- **Tests (TDD)** — `tests/search/test_sync.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_sync.py -v`

### Phase 2 — Config changes
> **Releasable**: after Task 2.2; `SearchConfig` carries `collections` and `history_collection` is derived

#### Task 2.1 — Add `collections` field to `SearchConfig`
...
- **Description**:
  ...
  - Remove `history_collection: str` field from `SearchConfig` dataclass
  - Update `load_config()` TOML parsing: read `collections` from `search_data.get("collections", SearchConfig().collections)`; remove `history_collection` parsing
  - In `load_config()`: if `search_data` contains `history_collection` key, log a WARNING and queue a Telegram notification (via the gateway notification channel) — the key is ignored
...
- **Tests (TDD)** — `tests/config/test_search_config.py`:
  - Unit: `test_search_config_default_collections`
  - Unit: `test_search_config_parses_collections_from_toml`
  - Unit: `test_search_config_history_collection_field_removed`
  - Unit: `test_search_config_warns_on_legacy_history_collection_key`
  - Checkpoint: `uv run pytest tests/config/ -v`

#### Task 2.2 — Replace `history_collection` references with derived value
- [x] **Files**: `archon/search/server.py`, `archon/search/pipeline.py`, `archon/gateway/gateway.py`, `archon/cli/search_cmd.py`, `archon/search/install.py`
- **Depends on**: Task 1.1, Task 2.1
- **Description**:
  - Remove all `cfg.search.history_collection` references
  - Replace with: `from archon.search.sync import path_to_collection_name; history_col = path_to_collection_name(str(Path(cfg.history.directory).expanduser() / "sessions"))`
  - **`SearchPipeline` constructor**: remove the `history_collection` parameter from `SearchPipeline.__init__` and the `create_pipeline` factory function; update all call sites in `server.py` and `gateway.py` to derive the name via `path_to_collection_name(...)`
  - In `server.py:create_app(pipeline, default_collection)` call: pass the derived name
  - In `gateway.py`: compute the derived name when building `search_url` context
  - In `search_cmd.py:_run_ingest`: use derived name as fallback when `--collection` not given
  - In `install.py`: remove `create_history_collection()`; replace call in `run()` with `SearchCollectionSync.sync(cfg.collections)`
- **Releasable**: all callers use derived history collection name; no `AttributeError` at startup
- **Tests (TDD)** — `tests/gateway/test_search_integration.py`, `tests/search/test_install.py`:
  ...
  - Checkpoint: `uv run pytest tests/gateway/ tests/search/ -v`

### Phase 3 — Service startup sync
> **Releasable**: after Task 3.1; service auto-syncs collections on every start

#### Task 3.1 — Call `SearchCollectionSync.sync()` in `server.py:main()`
...
- **Description**:
  - After `await pipeline.store.connect()`, before `await app.run_http_async(...)`:
    - Wrap `sync()` in `asyncio.wait_for(..., timeout=cfg.search.sync_timeout_seconds)`
...
#### Task 4.1 — Add `archon search sync` CLI command
- [x] **File**: `archon/cli/search_cmd.py`
- **Depends on**: Task 1.3, Task 2.1
- **Description**:
  - `run_search(args, search_parser, collection_parser)` — add two parser params for help commands (see Task 4.5)
  - Add `"sync": _run_sync` to `dispatch` in `run_search()`
  - `_run_sync(args)`:
    - Warn if service is running (write conflicts possible)
    - Load config, create pipeline, connect store, call `SearchCollectionSync(pipeline).sync(cfg.search.collections)`, disconnect
...
- **Releasable**: `archon search sync` reconciles all collections from CLI
- **Tests (TDD)** — `tests/cli/test_search_cmd.py`:
  ...
  - Checkpoint: `uv run pytest tests/cli/test_search_cmd.py -k "test_sync" -v`

#### Task 4.2 — Add `archon search collection list` CLI command
- [x] **File**: `archon/cli/search_cmd.py`
- **Depends on**: Task 1.1, Task 2.1
- **Description**:
  ...
  - `_run_collection_list(args)`:
    ...
    - Three status values per collection:
      ...
      - `unmanaged` — name in LanceDB but NOT in manifest (created via `archon search ingest --collection`); will NOT be touched by sync
...
- **Releasable**: `archon search collection list` shows full state
- **Tests (TDD)** — `tests/cli/test_search_cmd.py`:
  ...
  - Checkpoint: `uv run pytest tests/cli/test_search_cmd.py -k "collection_list" -v`

#### Task 4.3 — Add `archon search collection add <path>` CLI command
- [x] **File**: `archon/cli/search_cmd.py`
- **Depends on**: Task 1.3, Task 2.1
- **Description**:
  ...
  - Check `get_search_service().status().running`; if running: print a warning about write conflicts (do not block — just warn)
  ...
  - Immediately ingest: create pipeline, connect, call `pipeline.ingest_directory(resolved_path, col_name)`, disconnect
    - `col_name` is derived via `path_to_collection_name(path)` (collision-unaware). If the manifest already contains an entry for the same path (from a previous sync), use the existing name from the manifest; otherwise derive via `path_to_collection_name` (the name will be collision-checked on the next sync). If a collision with another collection exists, the naive name is used and will be resolved on the next `archon search sync`.
  - On ingest error: print error, but path remains in config (will retry on next sync); exit 1
  - Print: `"Collection added and indexed: <path>"` + `"Run 'archon search stop && archon search start' for the service to start serving it."`
  - `_config_collections_append(config_path: Path, path: str) -> None`: reads config.toml with tomlkit, appends to `[search] collections` using native Array, writes back atomically
  - `_manifest_lookup_by_path` renamed to `manifest_lookup_by_path` (public) in `archon/search/sync.py`
- **Releasable**: `archon search collection add <path>` registers and ingests a directory
- **Tests (TDD)** — `tests/cli/test_search_cmd.py`:
  ...
  - [x] Unit: `test_config_collections_append_creates_missing_search_section` *(added during DA review)*
  - [x] Unit: `test_collection_add_nonexistent_directory_ingest_fails` *(added during DA review)*
  - Checkpoint: `uv run pytest tests/cli/test_search_cmd.py -k "collection_add" -v`

#### Task 4.4 — Add `archon search collection remove <path>` CLI command
- [x] **File**: `archon/cli/search_cmd.py`
...
- **Releasable**: `archon search collection remove <path>` deregisters and drops a collection
- **Tests (TDD)** — `tests/cli/test_search_cmd.py`:
  ...
  - Checkpoint: `uv run pytest tests/cli/test_search_cmd.py -k "collection_remove" -v`

#### Task 4.5 — Add `archon search help` and `archon search collection help`
- [x] **Files**: `archon/cli/main.py`, `archon/cli/search_cmd.py`
- **Depends on**: Task 4.1, Task 4.2, Task 4.3, Task 4.4
- **Description**:
  - [x] `main.py`: add `search_sub.add_parser("help", help="Show search subcommand help")` and `collection_sub.add_parser("help", help="Show collection subcommand help")`
  - [x] Pass `p_search` and `p_collection` into `run_search(args, search_parser=p_search, collection_parser=p_collection)` at the call site in `main.py`
  - [x] `search_cmd.py:run_search(args, search_parser, collection_parser)`:
    - [x] `"help"` dispatch → `search_parser.print_help(); return 0`
  ...
  - [x] `main.py`: also register `search_sub.add_parser("sync")` and the full `collection` sub-tree (see Architecture section)
  - [x] If `args.search_command is None`: print search help and return 0 (consistent with top-level `help` handling)
- **Releasable**: `archon search help` and `archon search collection help` work; missing subcommand shows help instead of silent error
- **Tests (TDD)** — `tests/cli/test_search_cmd.py`, `tests/cli/test_main.py`:
  - [x] Unit: `test_search_help_prints_usage`
  - [x] Unit: `test_collection_help_prints_usage`
  - [x] Unit: `test_search_no_subcommand_prints_help`
...
- [x] **Files**: `Documentation/UserManual/search_guide.md`, `examples/config.toml.example`, `Documentation/Architecture/180_search_architecture.md`
- **Depends on**: Task 4.5
- **Description**:
  - `search_guide.md`:
    - "Declarative Collections" section — default paths, how config drives sync, restart vs `archon search sync`
  ...
  - `180_search_architecture.md`: update collection management section to describe sync model and CLI flow
- **Tests (TDD)**: N/A
