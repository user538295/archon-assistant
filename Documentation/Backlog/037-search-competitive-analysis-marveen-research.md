...
### Archon
**Design:** Layered service architecture with strict separation of concerns. Storage → Pipeline → Router → Server → CLI/Toolkit. Each layer has a single responsibility. FastMCP HTTP server decouples the search service from the main daemon. Pluggable backends via Protocol-typed interfaces (`EmbedderBackend`, `RerankerBackend`).

**Strengths:**
- Clean layering; each component is independently testable
- FastMCP decoupling means the search server survives main daemon restarts
...
- `archon_toolkit_search.py` is 884 lines — approaching god-object territory
...
### Archon
**What it does:** Full document ingestion: file discovery (15+ formats, 58 binary exclusions, symlink-skipping, hidden-file filtering) → format-specific parsing (trafilatura/docling/markitdown) → Chonkie RecursiveChunker (512-token, GPT-2 tokenizer) → fastembed embedding → LanceDB persist → FTS rebuild. SHA256 content addressing gives idempotent re-ingest. Declarative sync with file change detection and crash-recovery state machine.

**Strengths:**
- 15+ parsed formats including PDF (with OCR), Office docs, HTML
...
- `DocumentParser` is not thread-safe (explicitly documented); limits parallelism
...
### Archon
**Pipeline:** embed query → hybrid search (vector + FTS via LanceDB) → RRF fusion (k=60) → cross-encoder reranking (BAAI/bge-reranker-v2-m3) → context-window enrichment (adjacent chunk fetch).
...
### Archon
- **Default:** `BAAI/bge-small-en-v1.5` (384-dim, 33 MB ONNX, no GPU required)
- **Reranker:** `BAAI/bge-reranker-v2-m3` (cross-encoder)
- **Provider:** fastembed (CPU / CUDA / CoreML) — auto-detected by `SearchInstaller.detect_gpu()`
- **Configurable:** Both models are `config.toml` fields; can swap without code changes
- **Lazy loading:** Thread-safe singleton; loaded on first call
- **Validation:** `install.py validate_providers()` tests the embedding stack before committing config
...
**Strengths:**
- ANN index means sub-millisecond vector search at millions of records
- Native hybrid search (vector + FTS) without in-process merging
- Columnar storage — efficient for high-dimensional vector scans
- Metadata table with centroid, doc count, embedding model — enables multi-collection routing
...
- **ArchonToolkit:** 10 MCP tools registered into Claude sessions (start, stop, ingest, sync, collection lifecycle)
- **CLI:** `archon search <subcommand>` — 8 subcommands including `collection add/remove/reindex`
- **Health endpoint:** `GET /health` on the FastMCP HTTP server
...
**Weaknesses:**
- No REST API (only MCP protocol) — harder to integrate with non-Claude tools
- No streaming search results
- No authentication on the FastMCP server (assumed localhost-only)
...
### Archon
- **Install:** `SearchInstaller` wizard with GPU detection, provider validation, config write-back
- **Config:** 20+ fields in `[search]` section, all validated at load time, full annotated example
- **State recovery:** `IN_PROGRESS → PENDING` on restart (explicit crash recovery)
- **Health checks:** `archon doctor` checks service status, collection health, indexing state; `GET /health` HTTP endpoint
- **Filesystem watching:** Optional `watch = true` config; debounced per-collection triggers
- **Monitoring:** `IndexingNotificationMonitor` sends Telegram summary when all collections reach terminal state
- **Service management:** macOS launchd / Linux systemd via `PlatformService` ABC

**Strengths:**
- Crash recovery is explicit and tested
- `archon doctor` gives actionable pre-flight output
- Telegram notification on indexing completion is genuinely useful UX
- Platform-agnostic service management via strategy pattern

**Weaknesses:**
- No log rotation for the search server itself (main archon daemon handles this)
...
### Archon
- **Test lines:** 13,843 across 18 search test modules
...
|-------------|-------|---------|
| test_sync.py | 4,613 | Sync logic, change detection, state recovery |
| test_install.py | 1,892 | Dependency checks, GPU detection, provider config |
| test_server.py | 1,611 | FastMCP tools, HTTP endpoints, startup flow |
| test_pipeline.py | 1,172 | Ingest orchestration, description generation |
| test_store.py | 1,024 | LanceDB operations, hybrid search, metadata |
| test_progress.py | 874 | State serialization, ETA computation |
| test_watcher.py | 711 | Filesystem event debounce, callback scheduling |
| test_parser.py | 327 | Format-specific parsing (HTML, PDF, Office) |
| test_router.py | 325 | Centroid ranking, confidence gating |
| test_notification_monitor.py | 399 | Notification lifecycle |
| test_reranker.py | 197 | Cross-encoder scoring |
| test_embedder.py | 174 | Fastembed wrapper, lazy loading |
| test_description_generator.py | 172 | Haiku description generation |
| test_types.py | 111 | Dataclass serialization |
| test_chunker.py | 95 | Chonkie chunking |

**Strengths:**
- 18 test modules including sync, install, server, pipeline, store, progress, watcher, router, notification
...
**Weaknesses:**
- No coverage threshold enforcement visible in CI config
- Parser thread-safety issue is documented but not tested (accepted limitation)
...
| Memory per chunk | ~1.5 KB (384-dim float32) |
...
**Strengths:**
- LanceDB ANN scales to millions of documents without latency degradation
- `asyncio.to_thread()` keeps embedding non-blocking on the event loop
- `max_parallel_collections` config allows tuning for available resources
...
| Indexing notification | Telegram summary when all collections reach terminal state |
...
Archon currently does a full FTS rebuild on every `ingest_directory()` call. Switch to incremental FTS updates: LanceDB's `table.create_index(..., replace=False)` is additive. The bottleneck is only in bulk directory ingestion — individual `ingest_file()` already skips the rebuild.
