...
- `openai-whisper` model weights (~75MB–1.5GB) download on **first use**, not during `archon voice install`; users are warned during install.
...
class VoiceInstaller:
    ...
    def check_whisper(self) -> bool         # importlib.import_module("whisper"); False on ImportError
...
- **`test_check_whisper_importable`** (unit): patch `importlib.import_module` → success → True
...
- **`test_configure_preserves_comments`** (unit): `# my comment` survives tomlkit round-trip
- **`test_configure_missing_config_logs_warning`** (unit): non-existent file → warning logged, no raise
...
- **`test_voice_tools_registered_in_toolkit`** (unit): construct `ArchonToolkit`; assert 3 tool names present
...
- **`test_offer_voice_setup_happy_path`** (unit): all subprocesses rc=0 → `console.success` called
- **`test_offer_voice_setup_install_fails`** (unit): voice install rc=1 → `console.warn`, no config set
- **`test_offer_voice_setup_config_set_fails`** (unit): config set rc=1 → warn, no restart
...
- [ ] **File**: `archon/voice/__init__.py` (empty)
...
- **Description**:
  ...
  - `check_whisper(self) -> bool`
    - `importlib.import_module("whisper")` → True; `ImportError` → False
...
- **Tests (TDD)** — `tests/voice/test_install.py`:
  - Unit: `test_check_whisper_importable` — patch `importlib.import_module` to succeed → True
...
- **Depends on**: Task 1.2
- **Description**:
  - `configure_stt_model(self, model: str) -> None`
    - `path = Path(self._config_file)`
    - If `not path.exists()`: `logging.getLogger("archon").warning("Config file %s not found — skipping stt model config", path)` and return
    - `doc = tomlkit.parse(path.read_text(encoding="utf-8"))`
    - If `"voice" not in doc`: `doc["voice"] = tomlkit.table()`
    - If `"stt" not in doc["voice"]`: `doc["voice"]["stt"] = tomlkit.table()`
    - `doc["voice"]["stt"]["model"] = model`
    - `path.write_text(tomlkit.dumps(doc), encoding="utf-8")`
    - Preserves all existing comments (tomlkit round-trip)
- **Releasable**: after this task, `VoiceInstaller().configure_stt_model("medium")` writes config safely
- **Tests (TDD)** — `tests/voice/test_install.py`:
  - Unit: `test_configure_writes_model` — minimal config in `tmp_path`; assert `voice.stt.model == "tiny"` after `configure_stt_model("tiny")`
  - Unit: `test_configure_creates_voice_section` — config without `[voice]` → section + key created; valid TOML via `tomllib.loads`
  - Unit: `test_configure_creates_stt_subsection` — `[voice]` present but no `[voice.stt]` → stt subsection created
  - Unit: `test_configure_preserves_comments` — config with `# top comment` above `[access]` → comment present after write
  - Unit: `test_configure_missing_config_logs_warning` — non-existent path → warning logged, no exception raised
...
- **Description**:
  ...
  - Full flow:
    ...
    if self.check_whisper():
        ...
        print("[1/3] Installing openai-whisper (model weights ~75MB–1.5GB download on first use)…")
        try:
            self.install_deps()
    ...
    if not non_interactive:
        ...
        model = model_input if model_input in {"tiny", "small", "medium", "large", "base"} else "medium"
    else:
        model = "medium"
...
- **Tests (TDD)** — `tests/ai/test_archon_toolkit_voice.py`:
  - Unit: `test_voice_tools_registered_in_toolkit` — construct `ArchonToolkit()`; assert `"voice_status"`, `"voice_enable"`, `"voice_disable"` present in registered tool names
...
- **Description**:
  - `_offer_voice_setup(paths: InstallerPaths, console: Console, non_interactive: bool) -> None`
    ...
    - `console.info("Installing voice dependencies (~75MB–1.5GB model data on first use)…")`
    - Wrap the following in `except OSError as exc: console.warn(f"Voice setup failed: {exc}. Run 'archon voice install' to retry.")`:
      ```
      result = subprocess.run([archon_bin, "voice", "install", "--non-interactive"], check=False)
      if result.returncode != 0:
          console.warn("Voice installation failed. Run 'archon voice install' to retry.")
          return
      rc_cfg = subprocess.run([archon_bin, "config", "set", "voice.enabled", "true"], check=False).returncode
      if rc_cfg != 0:
          console.warn("Failed to enable voice. Run: archon config set voice.enabled true")
          return
      rc_restart = subprocess.run([archon_bin, "restart"], check=False).returncode
      if rc_restart != 0:
          console.warn("Failed to restart Archon. Run: archon restart")
          return
      ```
    - `console.success("Voice enabled. Archon is restarting — voice features will be available shortly.")`
...
- **Tests (TDD)** — `tests/test_installer_py.py` (new class `TestOfferVoiceSetup`):
  ...
  - Unit: `test_offer_voice_setup_happy_path` — all subprocess rc=0 → `console.success` called
  - Unit: `test_offer_voice_setup_install_fails` — voice install rc=1 → `console.warn`, config set not called
  - Unit: `test_offer_voice_setup_config_set_fails` — install ok, config set rc=1 → warn, restart not called
