# FIX-024 — Search install targets wrong virtualenv
**Purpose**: Fix `archon search install` so Search dependencies land in the Python environment the service actually runs under.
**Audience**: End users who install Archon from scratch and then run `archon search install`.
**Status**: Done
...
with a hardcoded shebang pointing to `~/.archon/app/.venv/bin/python3`. The Search launchd/systemd
service is registered with the same Python (via `sys.executable` in `search_service.register()`).

However, `SearchInstaller.install_deps()` calls `uv pip install` **without `--python`**. Without
that flag, `uv pip` uses its own venv-discovery logic (walks up from CWD, checks `$VIRTUAL_ENV`).
When the user runs `archon search install` from their development directory, `uv pip` finds the dev
...
2. The existing `search_service.register()` already uses `sys.executable` for the same purpose.

## Goal

After this fix, `archon search install` (run as the installed CLI) always installs Search packages
into the same Python environment the Search service runs under, eliminating the
...
### In Scope
- Add `import sys` to `archon/search/install.py`
...
- No shared `target_python()` helper is introduced (KISS — both `install_deps` and
  `search_service.register()` already use `sys.executable` independently; no extra abstraction needed).
- Linux `search_service.py` and Windows stubs are not modified — the `install_deps` fix is
  platform-agnostic and benefits Linux automatically.

---

## Acceptance criteria
- [ ] `archon search install` (via installed CLI) installs lancedb into `~/.archon/app/.venv`
- [ ] `~/.archon/app/.venv/bin/python3 -c "import lancedb"` exits 0 after running the installer
- [ ] `archon search status` no longer shows `No module named 'lancedb'`
- [x] All `TestInstallDeps` tests pass and include `--python sys.executable` assertions
- [x] All existing tests pass (`uv run pytest tests/search/test_install.py --no-cov`)

---

## What does NOT change
- `check_deps()` — still uses `importlib.import_module()` in the current interpreter
- `search_service.register()` — already uses `sys.executable`, unchanged
...
- All other `SearchInstaller` methods and the full `run()` / `run_uninstall()` flows

---

## Known limitations / accepted trade-offs
- If a developer runs `archon search install` from a dev venv (not the installed CLI), `sys.executable`
...
**Change**: `archon/search/install.py`
...
New tests added to `TestInstallDeps` in `tests/search/test_install.py`:
...
- [x] **Files**: `tests/search/test_install.py`, `archon/search/install.py`
- **Depends on**: nothing
- **Description**:
  **Step A — Write failing tests first (TDD red phase):**
  Add to `TestInstallDeps` in `tests/search/test_install.py`:
...
- **Releasable**: after this task, `archon search install` installs into the correct venv and the regression is locked in by tests
- **Tests (TDD)** — `tests/search/test_install.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_install.py --no-cov -v`
