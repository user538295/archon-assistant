...
## Acceptance criteria
- [ ] `uv pip install -e ".[search]"` installs all RAG dependencies without errors
...
- [ ] `python -m archon.search.server` starts a FastMCP HTTP server and exposes all 7 tools
...
- [ ] Archon gateway connects to the RAG server if `[search] enabled = true` and server is running
- [ ] Archon gateway logs a warning and continues without RAG if server is unreachable
- [ ] `startup_context_prompt(search_enabled=True)` references the `search` MCP tool by name
- [ ] `archon search status` shows service state and collection statistics
- [ ] `archon search ingest` re-ingests the history collection; `archon search ingest /path --collection my-docs` ingests a custom path
- [x] All `qmd` symbols removed from codebase; no references remain
- [ ] Test coverage ≥ 85% for `archon/search/`; all existing tests continue to pass
...
- [ ] `examples/config.toml.example`, section `[search]`: add full annotated `[search]` block, remove `[qmd]` block, path: `examples/config.toml.example`
...
- [ ] CLAUDE.md, `[search]` config fields table: update to reflect new section name and fields, path: `CLAUDE.md`
- [ ] Archive research doc (done): `Documentation/Completed/26_search_integration_research.md`
