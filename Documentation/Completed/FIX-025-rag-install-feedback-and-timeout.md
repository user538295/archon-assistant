# FIX-025 — Search install: silent progress and 30-second timeout
**Purpose**: Fix two bugs in `archon search install`: silent output after `uv pip install` ends, and a race condition that causes "Search service did not become ready within 30 seconds."
...
**Problem 2 — "Search service did not become ready within 30 seconds."** The root cause is a race condition between two independent 30-second timers:
...
After this fix: `archon search install` shows a numbered step label before every major operation (including a clear message before the model download), displays progress dots during the service readiness poll, and always completes successfully on first install instead of reporting a false timeout error.
...
- Add numbered step labels (`[1/5] … [5/5]`) and status messages to `run()` in `archon/search/install.py`
- Add progress dots in `_wait_for_service()` and increase its default timeout from 30 s to 60 s
- Update `tests/config/test_search_config.py` default assertion
- Update `examples/config.toml.example`, `Documentation/UserManual/search_guide.md`, and `Documentation/Architecture/180_search_architecture.md`
...
## Acceptance criteria
- [x] `archon search install` prints a status line before `validate_providers()` (the slow model-download step)
- [x] `archon search install` shows numbered step labels `[1/5]` through `[5/5]` before each major operation
...
- [x] `SearchConfig.sync_timeout_seconds` default is `0`
...
- `tests/search/test_server.py` — tests explicitly set `sync_timeout_seconds=5`; they are unaffected by the default change
- `SearchInstaller.run_uninstall()` — no progress changes needed
...
No new modules. All changes are within `archon/search/install.py` and `archon/config/loader.py`.

**`archon/config/loader.py`**
- `SearchConfig.sync_timeout_seconds: int` — default changes from `30` to `0`
...
- **`test_search_config_default_sync_timeout_is_zero`** (unit): `SearchConfig()` default `sync_timeout_seconds` is `0`
...
- **`test_run_prints_error_message_when_service_not_ready`** (unit): mock `_wait_for_service` to return `False`; capture stdout via `capsys`; assert `f"Search service did not become ready within {_WAIT_FOR_SERVICE_TIMEOUT} seconds."` (i.e. `"within 60 seconds"`) appears in stdout
...
- **`test_dry_run_does_not_print_wait_for_service_output`** (unit): create `SearchInstaller(dry_run=True)`; mock all side-effectful methods (`detect_gpu`, `_is_service_running`, `check_deps`, `configure_providers`, `write_service_file`, `load_service`, `_bootstrap_collections`) before calling `run()`; assert `"Waiting for Search service"` does NOT appear in stdout
- **`test_wait_for_service_keyboard_interrupt_prints_newline`** (unit): mock `_is_service_running` to raise `KeyboardInterrupt`; capture stdout; assert `KeyboardInterrupt` propagates (use `pytest.raises`); assert stdout contains a newline after `"Waiting for Search service"`

---

## Documentation update
- [x] `examples/config.toml.example`, section `[search]`, path: `examples/config.toml.example` — update `sync_timeout_seconds` from `30` to `0`
- [x] `Documentation/UserManual/search_guide.md`, section "Configuration reference" + "Startup sync behaviour", path: `Documentation/UserManual/search_guide.md` — update default from `30` to `0`; update prose to say `0` is the recommended default
- [x] `Documentation/Architecture/180_search_architecture.md` — update `sync_timeout_seconds: int = 30` to `= 0` in the SearchConfig dataclass listing; update `_wait_for_service(timeout=30)` to `timeout=60`
...
- **Description**:
  - `SearchConfig.sync_timeout_seconds: int = 0` (was `30`)
...
- **Releasable**: after this task, fresh `archon search install` no longer times out waiting for HTTP
- **Tests (TDD)** — `tests/config/test_search_config.py`:
  - Unit: `test_search_config_default_sync_timeout_is_zero` — `SearchConfig().sync_timeout_seconds == 0`; **update** existing `test_search_config_default_sync_timeout` assertion from `30` to `0`
  - Checkpoint: `uv run pytest tests/config/test_search_config.py --no-cov -v`
...
- **Description**:
  ...
  - Before `write_service_file()` + `load_service()`, print `[5/5] Starting Search service ...`
...
- **Tests (TDD)** — `tests/search/test_install.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_install.py --no-cov -v -k "test_run_prints"`
...
- **Description**:
  ...
  - Before the loop: `print("Waiting for Search service", end="", flush=True)`
  ...
  - Update the error message at the call-site (currently reads "Search service did not become ready within 30 seconds") to: `f"Search service did not become ready within {_WAIT_FOR_SERVICE_TIMEOUT} seconds."` (uses the module-level constant, not a local variable)
...
- **Tests (TDD)** — `tests/search/test_install.py`:
  - Unit: `test_wait_for_service_default_timeout_is_60` — inspect `SearchInstaller._wait_for_service.__defaults__` == `(60,)` OR assert `_WAIT_FOR_SERVICE_TIMEOUT == 60`
  ...
  - Unit: `test_run_prints_error_message_when_service_not_ready` — mock `_wait_for_service` to return `False`; capture stdout via `capsys`; assert `f"Search service did not become ready within {_WAIT_FOR_SERVICE_TIMEOUT} seconds."` (i.e. `"within 60 seconds"`) appears in stdout
  ...
  - Unit: `test_dry_run_does_not_print_wait_for_service_output` — create `SearchInstaller(dry_run=True)`; mock all side-effectful methods (`detect_gpu`, `_is_service_running`, `check_deps`, `configure_providers`, `write_service_file`, `load_service`, `_bootstrap_collections`) before calling `run()`; assert `"Waiting for Search service"` does NOT appear in stdout
  - Unit: `test_wait_for_service_keyboard_interrupt_prints_newline` — mock `_is_service_running` to raise `KeyboardInterrupt`; capture stdout; use `pytest.raises(KeyboardInterrupt)`; assert stdout contains a newline after `"Waiting for Search service"`
  - Checkpoint: `uv run pytest tests/search/test_install.py --no-cov -v -k "test_wait_for_service"`
...
- [x] **File**: `examples/config.toml.example`, `Documentation/UserManual/search_guide.md`, `Documentation/Architecture/180_search_architecture.md`
...
- **Description**:
  ...
  - `Documentation/UserManual/search_guide.md` configuration table: update default column from `30` to `0`
  - `Documentation/UserManual/search_guide.md` "Startup sync behaviour" section: update prose — change "default 30" to "default 0" and note that `0` is recommended for reliable install; note that setting a positive value blocks HTTP startup for that many seconds before responding to health checks
  - `Documentation/Architecture/180_search_architecture.md`: update the `SearchConfig` dataclass listing — change `sync_timeout_seconds: int = 30` to `= 0`; update any reference to `_wait_for_service(timeout=30)` to reflect `timeout=60`
- **Releasable**: after this task, docs match implementation
- **Tests (TDD)**: N/A — documentation only
  - Checkpoint: N/A; verify manually with `grep sync_timeout_seconds examples/config.toml.example Documentation/UserManual/search_guide.md Documentation/Architecture/180_search_architecture.md`
