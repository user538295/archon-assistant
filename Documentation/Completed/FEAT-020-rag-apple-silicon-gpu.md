...
- Apple Silicon detection placed in `archon/platform/` behind `PlatformRuntime.detect_gpu_type()`; `archon/search/install.py` calls the platform layer — no `sys.platform` / `platform.machine()` checks in `archon/search/`
...
- `search_guide.md` updated with an Apple Silicon section
...
- [x] `detect_gpu()` in `archon/search/install.py` delegates to `get_runtime().detect_gpu_type()` — no `sys.platform` or `platform.machine()` calls in `archon/search/`
...
- [x] `search_guide.md` documents Apple Silicon detection, validation, and the manual override

---

## What does NOT change
- `ModelEmbedder`, `ModelReranker` — `providers` is already passed through correctly
- `SearchConfig.providers` field and config loading — already supports arbitrary provider lists
...
- ONNX Runtime CoreML fallback to CPU is silent at runtime after install. If CoreML breaks on an OS update, the user won't notice until they check `archon search status` or re-run the installer. Accepted: adding a runtime health check is a separate concern.
...
### `detect_gpu() -> Literal["cuda", "apple_silicon", "none"]` in `archon/search/install.py`

`archon/search/install.py` delegates entirely to the platform layer:
...
No `sys.platform`, `platform.machine()`, or `subprocess` calls in `archon/search/install.py`.
...
### `install_deps(gpu: Literal["cuda", "apple_silicon", "none"])` in `archon/search/install.py`
...
### `configure_providers(gpu: Literal["cuda", "apple_silicon", "none"])` in `archon/search/install.py`
...
### `validate_providers(providers: list[str]) -> bool` in `archon/search/install.py`
...
### No new config keys
`[search] providers` already exists.
...
- [x] `search_guide.md`, section: Apple Silicon GPU Acceleration, path: `Documentation/UserManual/search_guide.md`
...
- [x] **Files**: `archon/platform/types.py`, `archon/platform/__init__.py`, `archon/platform/macos/runtime.py`, `archon/platform/linux/runtime.py`, `archon/platform/windows/runtime.py`, `archon/search/install.py`
- **Depends on**: nothing
- **Description**:
  ...
  - Change `detect_gpu(self) -> bool` to `detect_gpu(self) -> Literal["cuda", "apple_silicon", "none"]` in `archon/search/install.py`; implementation delegates to `get_runtime().detect_gpu_type()` — no `sys.platform`, `platform.machine()`, or `subprocess` in `archon/search/`
- **Tests (TDD)** — `tests/platform/macos/test_runtime.py`, `tests/platform/linux/test_runtime.py`, `tests/platform/windows/test_runtime.py`, `tests/search/test_install.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_install.py tests/platform/ -k "detect_gpu" -v`
...
- **Tests (TDD)** — `tests/search/test_install.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_install.py -k "install_deps" -v`
...
- **Tests (TDD)** — `tests/search/test_install.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_install.py -k "configure_providers" -v`
...
- **Description**:
  ...
  - Second check: create `TextEmbedding(self.cfg.embedding_model, providers=providers)` and call `list(model.embed(["archon search test"]))` — catches any runtime failure.
...
- **Tests (TDD)** — `tests/search/test_install.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_install.py -k "validate_providers" -v`
