...
When a user sets `search.enabled = true` in config and restarts, the system should automatically detect the RAG state and start the service if it is installed but not running. If RAG is not installed, it should send a clear Telegram notification with actionable guidance. The installer should also include RAG discovery guidance.
...
| 2 | `search.enabled=true` + server not running → gateway only logs WARNING to file; nothing in Telegram; RAG silently disabled | `archon/gateway/gateway.py` lines 446–455 |
...
**Never** run `archon search install` (package installation) from inside the gateway. Reasons:
- `SearchInstaller.run()` calls `asyncio.run()` internally — cannot be called from within an active event loop
...
**What the gateway WILL auto-do**: If RAG packages are installed and the service is registered (plist/unit exists) but the server is not listening → call `get_search_service().start()` via `asyncio.to_thread()`. This is lightweight (seconds), safe, and respects the daemon contract.
...
| `NOT_REGISTERED` | Probe fails + packages installed + plist/unit missing | Send Telegram: run `archon search install` |
...
- [x] Task E.1: Add `_detect_search_state()` to `gateway.py`
- [x] Task E.2: Add `_auto_start_search_service()` to `gateway.py`
...
#### Task E.1: Add `_detect_search_state()` to `gateway.py`

**File(s)**: `archon/gateway/gateway.py`

**What**: Add an async function `_detect_search_state(cfg: SearchConfig) -> SearchState` where `SearchState` is a `str` enum with values `RUNNING`, `NOT_INSTALLED`, `NOT_REGISTERED`, `NOT_RUNNING`. Logic:
...
3. Check if service is registered via `get_search_service().is_installed()` → if False, return `NOT_REGISTERED`
...
**TDD Steps**:
1. In `tests/gateway/test_search_auto_start.py`, add `TestDetectSearchState`:
   ...
   - `test_returns_not_registered_when_packages_present_service_not_registered` — probe fails, lancedb importable, `get_search_service().is_installed()` returns `False`, asserts `NOT_REGISTERED`
   - `test_returns_not_running_when_packages_installed_service_registered` — probe fails, lancedb importable, `get_search_service().is_installed()` returns `True`, asserts `NOT_RUNNING`
2. Implement `SearchState` enum (or `Literal`) and `_detect_search_state()` in `gateway.py`
3. Run tests

**Acceptance criteria**:
- All four states correctly detected
- `find_spec` check does not import lancedb (just probes importability)
- `get_search_service()` is the platform-abstracted service singleton

---

#### Task E.2: Add `_auto_start_search_service()` to `gateway.py`

**File(s)**: `archon/gateway/gateway.py`

**What**: Add `async def _auto_start_search_service(host: str, port: int) -> bool` that:
1. Calls `exit_code = await asyncio.to_thread(get_search_service().start)` (non-blocking start)
...
2. Implement `_auto_start_search_service()` in `gateway.py`
3. Run tests

**Acceptance criteria**:
- `get_search_service().start()` is called via `asyncio.to_thread()` (never directly)
...
Part B — Wire into `Gateway._run()`: Replace the existing `_ensure_rag_server` probe block (lines 446–455) with this extended block, still BEFORE `Pipeline` and `BackgroundAgentManager` construction (which use `search_url`):
1. Call `state = await _detect_search_state(cfg.search)` (subsumes the existing TCP probe)
2. `auto_started = False`
3. If `state == RUNNING`: set `search_url = f"http://{cfg.search.host}:{cfg.search.port}/mcp"`
4. If `state == NOT_RUNNING`: call `auto_started = await _auto_start_search_service(cfg.search.host, cfg.search.port)`; if True, set `search_url = f"http://{cfg.search.host}:{cfg.search.port}/mcp"`
5. (For NOT_INSTALLED / NOT_REGISTERED / failed auto-start: `search_url` stays `None`)
...
**Critical constraint**: Steps 1–5 (all `search_url` updates) MUST occur before `Pipeline(search_url=search_url, ...)` and `BackgroundAgentManager(search_url=search_url, ...)` are constructed. These constructors take `search_url` as a constructor argument and do not observe mutations after construction.
...
**TDD Steps**:
1. In `tests/gateway/test_search_auto_start.py`, add `TestSearchStateNotification`:
   - `test_not_installed_message_sent_to_all_users` — state=NOT_INSTALLED, asserts message contains "archon search install" and "archon search start"
   - `test_not_registered_message_contains_install_command` — state=NOT_REGISTERED, asserts message contains "archon search install"
   - `test_not_running_auto_started_true_sends_success` — auto_started=True, asserts "✅" and "started automatically"
   - `test_not_running_auto_started_false_sends_failure` — auto_started=False, asserts "⚠️" and "archon search status"
...
2. In `tests/gateway/test_startup_notification.py`, add wiring tests:
   - `test_gateway_auto_starts_when_state_is_not_running` — patches `_detect_search_state` → NOT_RUNNING, patches `_auto_start_search_service` → True, asserts `_auto_start_search_service` was called
   - `test_gateway_skips_auto_start_when_not_installed` — state=NOT_INSTALLED, asserts `_auto_start_search_service` NOT called
   - `test_gateway_updates_search_url_after_successful_auto_start` — auto_start succeeds, asserts `search_url` is set (not None) for the session
   - `test_gateway_no_notification_when_rag_disabled` — `cfg.search.enabled=False`, asserts `_register_rag_state_notification` not called
...
- `search_url` is updated when auto-start succeeds (session gets RAG context)
- `search_url` stays `None` when auto-start fails or state is not `RUNNING`
...
**What**: Extract the RAG server reachability check from `_check_rag_health()` into a new **synchronous** function `_check_rag_server(cfg: Config) -> CheckResult` using `socket.create_connection((cfg.search.host, cfg.search.port), timeout=2)`. Return values:

| Condition | `CheckResult` |
|-----------|---------------|
| `search.enabled = false` | `CheckResult("search server", True, "disabled")` |
| Enabled + lancedb not importable | `CheckResult("search server", False, "RAG not installed — run: archon search install")` |
| Enabled + packages installed, service not registered | `CheckResult("search server", False, "service not registered — run: archon search install")` |
| Enabled + probe fails (registered but not running) | `CheckResult("search server", False, "not running — run: archon search start")` |
| Enabled + probe succeeds | `CheckResult("search server", True, "running")` |
...
1. In `tests/cli/test_doctor.py`, add `TestCheckRagServer`:
   - `test_disabled_returns_ok` — `search.enabled=False`, asserts `result.ok is True`
   - `test_not_installed_returns_fail_with_install_guidance` — `find_spec("lancedb")` returns None, asserts fail + "archon search install"
   - `test_not_registered_returns_fail_with_install_guidance` — packages present, `get_search_service().is_installed()` False, asserts fail
   - `test_not_running_returns_fail_with_start_guidance` — packages present, registered, socket connect fails, asserts fail + "archon search start"
...
1. In `tests/cli/test_doctor.py`, add `TestRunDoctorRagExitCode`:
   - `test_returns_1_when_search_enabled_not_running` — all non-RAG checks ok, `_check_rag_server` returns fail, asserts `run_doctor()` returns 1
   - `test_returns_0_when_rag_disabled` — `search.enabled=False`, asserts returns 0
   - `test_prints_check_mark_x_for_rag_failure` — asserts `✗` and "search server" in stdout
...
**Acceptance criteria**:
- `run_doctor()` returns 1 when `search.enabled=true` and server not reachable
- Returns 0 when `search.enabled=false`
...
Optional: Enable semantic search (RAG)
  archon search install    # install RAG dependencies (~150MB)
  archon search start      # start the RAG service
  archon config set search.enabled true
  archon restart
```

**Why**: The installer is the primary discovery path. Adding a small guidance block costs nothing and ensures every user sees the path to RAG. No subprocess RAG install during the main installer — it's too heavy and risky (packages, downloads, event loop issues in the SearchInstaller).

**TDD Steps**:
1. In `tests/test_installer_py.py`, add `TestPostInstallRagGuidance`:
   - `test_success_message_always_includes_rag_guidance` — mocks a successful install run, captures output via `capsys`, asserts `"archon search install"` and `"archon search start"` appear
   - `test_search_guidance_present_in_non_interactive_mode` — `--non-interactive` flag, asserts guidance still appears
2. Update the success output block in `main()`
3. Run tests; confirm all existing installer tests pass

**Acceptance criteria**:
- Post-install output always contains `archon search install` and `archon search start`
...
- Auto-running `archon search sync` on startup (sync is user-initiated; server handles its own startup indexing)
