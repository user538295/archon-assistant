...
**Purpose**: Replace the Node.js QMD dependency with a fully Python-native, local-first RAG stack (`archon/search/`) that Claude Code can query via MCP for semantic + keyword search over conversation history and user-specified document collections.
...
Archon currently optionally integrates with QMD (`@tobilu/qmd`), a Node.js tool that provides vector search over conversation history. QMD requires Node.js ≥ 22 or Bun ≥ 1.0, downloads ~3 GB of GGUF models at first run, and runs as an HTTP MCP daemon. The Python ecosystem now provides a superior local-first stack (LanceDB + Docling + fastembed + FastMCP) that: eliminates the Node.js dependency, supports every required document format natively, delivers hybrid BM25+vector search with cross-encoder reranking, and integrates with Archon's existing MCP wiring unchanged. `fastembed` (by Qdrant) replaces `sentence-transformers` as the embedding/reranking backend — it uses ONNX Runtime instead of PyTorch, eliminating the 2-5 GB PyTorch dependency and the macOS process-explosion caused by the HuggingFace `tokenizers` Rust library spawning 100+ worker processes at import time. Full technology selection rationale is in [Documentation/Completed/26_search_integration_research.md](../Completed/26_search_integration_research.md).

**Note**: `Documentation/Completed/26_search_integration_research.md` needs to be created as part of Task 8.2 to document the technology selection rationale. Until then, the full rationale is contained in this spec's Background and Architecture sections.

## Goal

An operator runs `archon search install`, answers a few prompts, and afterward Claude Code has access to a `search` MCP tool backed by a persistent local daemon that: indexes conversation history automatically, accepts arbitrary document collections the user defines, and answers semantic + keyword queries entirely offline. The QMD config section, binary, and installer script are removed; `[search]` config replaces `[qmd]` with no migration path.
...
### In Scope
- New `archon/search/` sub-package: store, embedder, reranker, parser, chunker, pipeline, FastMCP server, installer
- Optional `[search]` extras group in `pyproject.toml` (install with `uv pip install -e ".[search]"`)
- `[search]` config section replacing `[qmd]` (no migration — clean break)
- `archon search` CLI subcommand group: `install`, `uninstall`, `start`, `stop`, `status`, `ingest`
- macOS launchd service `com.archon.search` (Linux systemd in phase 7, Windows stub only)
- 7 MCP tools: `search`, `search_with_context`, `ingest_file`, `ingest_directory`, `list_collections`, `list_documents`, `delete_document`
- `archon search ingest [path] [--collection name]` — defaults to history dir if no path given
- System prompt update: `search_enabled=True` text references the `search` tool specifically
- Full rename of all `qmd_url` / `qmd_enabled` symbols → `search_url` / `search_enabled`
...
## Acceptance criteria
- [x] `uv pip install -e ".[search]"` installs all RAG dependencies without errors
- [ ] `archon search install` completes on macOS: creates `~/.archon/search/`, registers `com.archon.search` launchd service, runs initial ingest of history collection
- [x] `python -m archon.search.server` starts a FastMCP HTTP server and exposes all 7 tools
...
- [x] Archon gateway connects to the RAG server if `[search] enabled = true` and server is running
- [x] Archon gateway logs a warning and continues without RAG if server is unreachable
- [x] `startup_context_prompt(search_enabled=True)` references the `search` MCP tool by name
- [x] `archon search status` shows service state and collection statistics
- [x] `archon search ingest` re-ingests the history collection; `archon search ingest /path --collection my-docs` ingests a custom path
- [x] All `qmd` symbols removed from codebase; no references remain
- [x] Test coverage ≥ 85% for `archon/search/`; all existing tests continue to pass
- [ ] `uv run mypy archon/` reports zero errors

---

## What does NOT change
- `ClaudeSession` MCP registration mechanism (`mcp_servers["search"] = {"type": "http", "url": ...}`) — only the key name changes from `"qmd"` to `"rag"`
- `ContextProvider` protocol shape — only the parameter name changes (`qmd_enabled` → `search_enabled`)
...
- **No auto re-index** — ingestion is manual (`archon search ingest`) or triggered via MCP tool. A future watcher can be added without changing the pipeline.
- **Windows: manual run only** — `archon search start/stop` stubs out on Windows with a clear message. The server itself (`python -m archon.search.server`) works on all platforms.
...
- **`archon search ingest` is not incremental v1** — re-ingesting a collection replaces all chunks for documents that have changed (identified by path hash). Full incremental diffing deferred.
...
├── store.py             # SearchStore — LanceDB async store (dataclasses in _types.py)
...
├── pipeline.py          # SearchPipeline: orchestrates store + embedder + reranker + chunker + parser
├── server.py            # FastMCP HTTP server; entry point: python -m archon.search.server
└── install.py           # SearchInstaller: deps, data dir, model download, service registration, ingest
...
tests/cli/test_search_cmd.py   # new file alongside existing CLI tests
```

Additional platform files added by this feature:
```
archon/platform/macos/search_service.py   # RagPlatformService for macOS (launchd)
archon/platform/linux/search_service.py   # RagPlatformService for Linux (systemd)
```
...
gateway.py
  ...
  └── cfg.search.enabled → builds search_url → SessionManager(search_url=...) + BackgroundAgentManager(search_url=...)

SessionManager(search_url)
  └── _create_session() → ClaudeSession(search_url=...)
        └── _build_mcp_servers() → mcp_servers["search"] = {"type": "http", "url": search_url}

SessionManager(search_url) → startup_context_prompt(search_enabled=search_url is not None)
  └── HistoryCompactor.startup_context_prompt(search_enabled=True) → includes search tool hint

archon search install → SearchInstaller
  └── writes com.archon.search.plist → launchctl load → launchctl start
```

### New config keys (`[search]` section)
...
### Phase 1 — Foundation
- **test_search_config_defaults** (unit): SearchConfig fields match documented defaults
- **test_search_config_custom_values** (unit): all fields parsed from TOML dict
- **test_search_config_invalid_port** (unit): port out of range raises ConfigError
- **test_search_config_missing_optional_uses_default** (unit): omitted keys use dataclass defaults
...
- **test_search_config_replaces_qmd_config** (unit): Config.rag exists; Config.qmd does not
...
- **test_claude_session_registers_rag_mcp_server** (unit): search_url set → mcp_servers["search"] built
- **test_claude_session_skips_rag_when_none** (unit): search_url=None → "rag" key absent from mcp_servers
- **test_session_manager_passes_search_url** (unit): SessionManager(search_url=...) → ClaudeSession receives it
- **test_startup_prompt_search_enabled_mentions_search_tool** (unit): search_enabled=True → text contains "search"
- **test_startup_prompt_rag_disabled_omits_search_tool** (unit): search_enabled=False → no search tool mention
- **test_background_agent_manager_passes_search_url** (unit): BAM(search_url=...) → ClaudeSession receives it
- **test_gateway_rag_lifecycle_integration** (integration): gateway starts → RAG unreachable → logs warning → continues; next session has no search_url
...
- **test_write_service_file_delegates_to_platform** (unit): `get_search_service().register()` called
- **test_write_service_file_dry_run** (unit): dry_run=True → `register(dry_run=True)` called
- **test_search_platform_service_macos_plist_contains_label** (unit): plist XML contains `com.archon.search`
- **test_search_cmd_install_calls_installer** (unit): `archon search install` → SearchInstaller.run() called
- **test_search_cmd_start_calls_service_start** (unit): `archon search start` → platform service start
- **test_search_cmd_stop_calls_service_stop** (unit): `archon search stop` → platform service stop
- **test_search_cmd_status_prints_service_info** (unit): `archon search status` → output contains "running" or "stopped"
- **test_search_cmd_ingest_no_args_uses_history_dir** (unit): no path arg → history_dir used
- **test_search_cmd_ingest_with_path_and_collection** (unit): `--collection my-docs /path` → correct args passed
- **test_installer_register_service_linux_writes_unit_file** (unit): Linux path → systemd unit written at expected path
- **test_search_cmd_status_server_unreachable** (unit): server unreachable → status command prints "unreachable", returns non-zero
- **test_search_start_calls_platform_service** (unit): `archon search start` → delegates to `get_search_service().start()`
- **test_search_stop_calls_platform_service** (unit): `archon search stop` → delegates to `get_search_service().stop()`
- **test_search_status_prints_service_state** (unit): output contains "running" or "stopped"
- **test_search_status_server_unreachable_prints_warning** (unit): server down → "unreachable" in output, non-zero return

---

## Documentation update
- [x] `examples/config.toml.example`, section `[search]`: add full annotated `[search]` block, remove `[qmd]` block, path: `examples/config.toml.example`
...
- [x] CLAUDE.md, `[search]` config fields table: update to reflect new section name and fields, path: `CLAUDE.md`
- [x] Archive research doc (done): `Documentation/Completed/26_search_integration_research.md`
...
### Phase 1 — Foundation
> **Releasable**: after Task 1.4 — `SearchStore` is independently testable; LanceDB ingest and hybrid search work end-to-end with in-process data.

#### Task 1.1 — Optional `[search]` extras in `pyproject.toml`
...
- **Description**:
  ...
  - `fastembed` is the CPU baseline. The installer (Task 7.1) auto-detects GPU at install time and replaces `fastembed` with `fastembed-gpu` + `onnxruntime-gpu` if an NVIDIA GPU is found. The two packages conflict (`onnxruntime` vs `onnxruntime-gpu`) — the installer installs exactly one. The Python API is identical; GPU is activated by writing `providers = ["CUDAExecutionProvider"]` to `[search]` in `config.toml`.
...
- **Releasable**: after this task, `uv pip install -e ".[search]"` installs the full RAG stack
- **Tests (TDD)** — N/A (config-only change):
  - Checkpoint: `uv pip install -e ".[search]" --dry-run` (manual verification)

#### Task 1.2 — `SearchConfig` dataclass in `config/loader.py`
...
- **Description**:
  - Remove `QmdConfig` dataclass entirely; replace with:
    ...
    class SearchConfig:
        ...
        chunk_size: int = 512
    ```
  - `providers`: empty list = CPU (ONNX default); `["CUDAExecutionProvider"]` = GPU. Written automatically by `SearchInstaller` when a GPU is detected; users can also set it manually in `config.toml`. No validation needed — invalid providers fail at model init with a clear `onnxruntime` error.
  - Replace `Config.qmd: QmdConfig` → `Config.rag: SearchConfig`
...
- **Releasable**: after this task, `config.search` is available throughout the codebase
- **Tests (TDD)** — `tests/config/test_config_loader.py` (update existing `qmd` tests):
  - Unit: `test_search_config_defaults` — `SearchConfig()` fields match documented defaults
  - Unit: `test_search_config_all_fields_parsed` — full TOML dict parsed correctly
  - Unit: `test_search_config_invalid_port_raises` — port 0 and 65536 both raise `ConfigError`
  - Unit: `test_search_config_top_k_validation` — `top_k_return > top_k_retrieve` raises `ConfigError`
  - Unit: `test_config_has_no_qmd_attribute` — `Config` has no `.qmd` attribute
  - Unit: `test_search_config_chunk_size_zero_raises` — `chunk_size=0` raises `ConfigError`
  - Unit: `test_search_config_top_k_return_zero_raises` — `top_k_return=0` raises `ConfigError`
  - Unit: `test_search_config_missing_optional_uses_default` — TOML dict containing only `enabled = true` (all other keys absent); verify `SearchConfig` fields all use documented defaults
  - Checkpoint: `uv run pytest tests/config/ -v`

#### Task 1.3 — Shared dataclasses in `archon/search/_types.py`
- [x] **File**: `archon/search/_types.py`
- **Depends on**: nothing
- **Description**:
  - Define all shared dataclasses used across the RAG sub-package. No imports from other `archon/search/` modules — this is a leaf dependency.
...
- **Releasable**: after this task, all dataclasses are importable; all other `archon/search/` modules depend on this
- **Tests (TDD)** — `tests/search/test_types.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_types.py -v`

#### Task 1.4 — `SearchStore` in `archon/search/store.py`
- [x] **File**: `archon/search/store.py`
- [x] **File**: `archon/search/__init__.py` (empty)
- **Depends on**: Task 1.3 (`_types.py` — `store.py` imports `ChunkRecord`, `SearchResult`, `DocumentInfo`, `CollectionInfo` from it)
- **Description**:
  - **Note**: All shared dataclasses (`ChunkRecord`, `SearchResult`, `DocumentInfo`, `CollectionInfo`) are defined in `archon/search/_types.py` (Task 1.3), not in `store.py`. `store.py` imports from `_types.py`.
  ...
  - All methods raise `RuntimeError("SearchStore not connected")` if called before `connect()`
  - All LanceDB calls are natively async — no `asyncio.to_thread()` needed
  - **Import boundary rule**: All imports of optional dependencies (`lancedb`, `pyarrow`, `fastembed`, `sentence_transformers`, `docling`, `markitdown`, `trafilatura`, `chonkie`, `fastmcp`) in `archon/search/` modules MUST be either: (a) top-level imports within `archon/search/` module files (acceptable since these files are only loaded when the rag extras are installed), OR (b) lazy imports inside functions/methods (required for any `_parse_*` method per Task 3.1). The critical constraint is that NO module OUTSIDE `archon/search/` (e.g., `archon/cli/search_cmd.py`, `archon/gateway/gateway.py`) should import from `archon.search.*` at module level — all cross-package imports must be lazy (inside function bodies). This ensures that running `archon start` or `archon status` without the `[search]` extras installed does not raise `ImportError`.
...
- **Tests (TDD)** — `tests/search/test_store.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_store.py -v`
...
- **Description**:
  ...
  - Create `tests/search/__init__.py` (empty) alongside conftest.
...
- **Tests (TDD)** — `tests/search/test_conftest.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_conftest.py -v`

#### Task 2.1 — `Embedder` with injectable backend in `archon/search/embedder.py`
- [x] **File**: `archon/search/embedder.py`
- **Depends on**: nothing
- **Description**:
...
- **Tests (TDD)** — `tests/search/test_embedder.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_embedder.py -v`

#### Task 2.2 — `Reranker` with injectable backend in `archon/search/reranker.py`
...
- **Tests (TDD)** — `tests/search/test_reranker.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_reranker.py -v`

#### Task 2.3 — Migrate source files to `fastembed` and rewrite all `tests/search/` from scratch
- [x] **File**: `pyproject.toml`
- [x] **File**: `archon/search/embedder.py`
- [x] **File**: `archon/search/reranker.py`
- [x] **File**: `archon/config/loader.py`
- [x] **File**: `tests/search/__init__.py` (recreate)
- [x] **File**: `tests/search/conftest.py` (rewrite from scratch)
- [x] **File**: `tests/search/test_conftest.py` (rewrite from scratch)
- [x] **File**: `tests/search/test_types.py` (rewrite from scratch)
- [x] **File**: `tests/search/test_store.py` (rewrite from scratch)
- [x] **File**: `tests/search/test_embedder.py` (rewrite from scratch)
- [x] **File**: `tests/search/test_reranker.py` (rewrite from scratch)
- [x] **File**: `tests/search/test_parser.py` (rewrite from scratch)
- **Depends on**: Tasks 2.1, 2.2 (already complete)
- **Note**: The entire `tests/search/` directory was deleted because the old tests were written against `sentence-transformers` and caused CPU burnout / infinite loops (HuggingFace `tokenizers` Rust library spawning 100+ worker processes). All test files must be written from scratch targeting `fastembed` + `onnxruntime`. The source files (Tasks 2.1, 2.2) also need to be migrated.
- **Description**: Tasks 1.1, 1.2, 2.0, 2.1, 2.2, 3.1 were implemented against `sentence-transformers`. This task: (1) migrates source files to `fastembed`, and (2) rewrites all `tests/search/` files from scratch.

  **Source file migrations:**
  - **`pyproject.toml`**: replace `sentence-transformers>=3.0.0` with `fastembed>=0.7.4`; replace `chonkie[all]>=0.5.0` with `chonkie>=0.5.0`
  - **`archon/search/embedder.py`**: rewrite `ModelEmbedder` — replace `from sentence_transformers import SentenceTransformer` with lazy `from fastembed import TextEmbedding`; add `providers: list[str]` parameter to `__init__`; pass `providers=providers or None` to `TextEmbedding(model_name, providers=...)`; change `.encode(texts).tolist()` to `[e.tolist() for e in self._model.embed(texts)]` (embed returns a generator of 1-D numpy arrays). Update `make_embedder(model_name, providers)` factory accordingly.
  - **`archon/search/reranker.py`**: rewrite `ModelReranker` — replace `from sentence_transformers import CrossEncoder` with lazy `from fastembed import TextCrossEncoder`; add `providers: list[str]` parameter to `__init__`; pass `providers=providers or None` to `TextCrossEncoder(model_name, providers=...)`; change `.predict(pairs).tolist()` to `list(self._model.rerank(pairs[0][0], [p[1] for p in pairs]))` (single query str, returns `Iterable[float]`). Return `[]` on empty pairs. Update `make_reranker(model_name, providers)` factory accordingly.
  ...
  **Test files — write from scratch targeting fastembed:**
  - **`tests/search/__init__.py`**: empty file (package marker)
  - **`tests/search/conftest.py`**: block `fastembed` import at `sys.modules` level before any test runs (module-level injection, not a fixture). This prevents ONNX model download and any process explosion. Also set `os.environ.setdefault("TOKENIZERS_PARALLELISM", "false")`. Inject fake `fastembed` module: `_fake_fastembed.TextEmbedding` factory returns a mock whose `.embed()` yields 1-D zero numpy arrays of shape `(384,)` per input text (matching `TextEmbedding.embed()` generator contract). `_fake_fastembed.TextCrossEncoder` factory returns a mock whose `.rerank()` returns uniform `[0.5, 0.5, ...]` plain floats (not numpy). Keep a secondary `sentence_transformers` block as a belt-and-braces guard. The `connected_store` fixture is module-scoped (one LanceDB connection per test module) to avoid thread-pool explosion. Each test gets a unique `col_name` fixture.
  - **`tests/search/test_conftest.py`**: `test_fastembed_is_patched` — `import fastembed; fastembed.TextEmbedding(...)` completes without network access and returns a mock
  - **`tests/search/test_types.py`**: rewrite all `ChunkRecord`, `SearchResult`, `DocumentInfo`, `CollectionInfo`, `IngestResult` dataclass tests as specified in Task 1.3
  - **`tests/search/test_store.py`**: rewrite all store tests as specified in Task 1.4 (full list in the Task 1.4 test section)
  - **`tests/search/test_embedder.py`**: rewrite all embedder tests as specified in Task 2.1 (full list in the Task 2.1 test section)
  - **`tests/search/test_reranker.py`**: rewrite all reranker tests as specified in Task 2.2 (full list in the Task 2.2 test section)
  - **`tests/search/test_parser.py`**: rewrite all parser tests as specified in Task 3.1 (full list in the Task 3.1 test section)
...
- **Tests (TDD)**:
  - Checkpoint: `uv run pytest tests/search/ -v`
...
- **Tests (TDD)** — add to existing `tests/search/test_embedder.py` and `tests/search/test_reranker.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_embedder.py tests/search/test_reranker.py -v`
...
#### Task 3.1 — `DocumentParser` in `archon/search/parser.py`
...
- **Tests (TDD)** — `tests/search/test_parser.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_parser.py -v`

#### Task 3.2 — `DocumentChunker` in `archon/search/chunker.py`
- [x] **File**: `archon/search/chunker.py`
...
- **Tests (TDD)** — `tests/search/test_chunker.py`:
  ...
  - Checkpoint: `uv run pytest tests/search/test_chunker.py -v`

---

### Phase 4 — Pipeline
> **Releasable**: after Task 4.1 — `SearchPipeline` integrates all components; a file can be ingested and searched end-to-end using mock backends.

#### Task 4.1 — `SearchPipeline` in `archon/search/pipeline.py`
- [x] **File**: `archon/search/pipeline.py`
- **Depends on**: Tasks 1.3, 1.4, 2.1, 2.2, 3.1, 3.2 (Task 1.3 = `_types.py`, Task 1.4 = `SearchStore`)
- **Description**:
  - `IngestResult` is imported from `archon/search/_types.py` (Task 1.3) — not defined in `pipeline.py`
  - `class SearchPipeline`:
    - `__init__(self, store: SearchStore, embedder: Embedder, reranker: Reranker, chunker: DocumentChunker, parser: DocumentParser, history_collection: str, top_k_retrieve: int, top_k_return: int) -> None`
    - `async def ingest_file(self, path: Path, collection: str, rebuild_fts: bool = True) -> IngestResult`:
    ...
    - `store: SearchStore` — public attribute (store reference) exposed for lifecycle management by `server.py main()`
    - Factory: `def create_pipeline(cfg: SearchConfig, embedder_backend: EmbedderBackend | None = None, reranker_backend: RerankerBackend | None = None) -> SearchPipeline` — builds the full component graph from config. `embedder_backend` and `reranker_backend` default to `ModelEmbedder` and `ModelReranker` when not supplied (injectable for tests). Exported from `archon/search/pipeline.py`.
...
- **Tests (TDD)** — `tests/search/test_pipeline.py`:
  ...
  - Unit: `test_create_pipeline_wires_all_components` — call `create_pipeline(SearchConfig())` with mocked heavy deps; assert returned `SearchPipeline` has non-None `.store`, `.embedder`, `.reranker`, `.chunker`, `.parser` attributes
  - Unit: `test_create_pipeline_does_not_auto_connect` — call `create_pipeline(SearchConfig())`; immediately call a store method (e.g., `list_collections()`); assert `RuntimeError("SearchStore not connected")` raised (confirms factory does not call `store.connect()`)
  ...
  - Checkpoint: `uv run pytest tests/search/test_pipeline.py -v`

---

### Phase 5 — MCP Server
> **Releasable**: after Task 5.1 — `python -m archon.search.server` starts a live FastMCP HTTP server; all 7 tools are callable via MCP protocol.

#### Task 5.1 — FastMCP HTTP server in `archon/search/server.py`
...
- **Description**:
  - `def create_app(pipeline: SearchPipeline, default_collection: str) -> FastMCP`:
    - Creates `app = FastMCP("archon-search")`
    ...
    - All tools log to `logging.getLogger("archon.search")` (stderr in production)
    - Returns `app`
  - `async def main() -> None`:
    - Loads config via `load_config()` (finds `config.toml` via standard path logic)
    - Calls `pipeline = create_pipeline(cfg.search)` (factory from `archon/search/pipeline.py`) — assembles all components from config. The factory also creates and holds the `SearchStore` internally.
    - Calls `await pipeline.store.connect()` (or however the pipeline exposes the store for lifecycle management)
    - Creates app: `app = create_app(pipeline, cfg.search.history_collection)`
    - Calls `await app.run_async(transport="http", host=cfg.search.host, port=cfg.search.port)`. Use `run_async()` (not `run()`) since `main()` is an `async def` — `run()` calls `asyncio.run()` internally and would raise `RuntimeError` in an async context. Note: verify the exact transport parameter name against the installed FastMCP version (`pip show fastmcp`). FastMCP 3.x uses `"http"` not `"streamable-http"`.
...
- **Releasable**: after this task, `python -m archon.search.server` serves all 7 MCP tools
- **Tests (TDD)** — `tests/search/test_server.py`:
  ...
  - Integration: `test_server_search_tool_with_real_pipeline` — create `SearchPipeline` with mock ML backends and real store (tmp_path), ingest one file, call the `search` tool through FastMCP's `TestClient` (or equivalent test transport) to invoke the tool through the MCP protocol layer — not calling the underlying Python function directly. Verifies serialization and the full request-response cycle. → non-empty result list returned
  ...
  - Checkpoint: `uv run pytest tests/search/test_server.py -v`

---

### Phase 6 — Archon Integration
> **Releasable**: after Task 6.5 — Archon gateway connects to the RAG server when `[search] enabled = true`; Claude Code can call `search` in sessions; all previous QMD references removed.

#### Task 6.1 — Rename `qmd_url → search_url` in `ai/claude_session.py`
- [x] **File**: `archon/ai/claude_session.py`
- **Depends on**: Task 1.2
- **Description**:
  - Rename parameter `qmd_url: str | None = None` → `search_url: str | None = None`
  - Rename `self._qmd_url` → `self._search_url`
  - Change MCP server key: `mcp_servers["qmd"]` → `mcp_servers["search"]`
  - Condition: `if self._search_url is not None: mcp_servers["search"] = {"type": "http", "url": self._search_url}`
...
- **Tests (TDD)** — `tests/ai/test_claude_session.py` (update existing tests):
  - Unit: `test_search_url_registers_mcp_server` — `search_url` set → `mcp_servers["search"]` built
  - Unit: `test_search_url_none_omits_mcp_server` — `search_url=None` → no "rag" key
...
#### Task 6.2 — Rename `qmd_url → search_url` in `ai/background_agent_manager.py`
- [x] **File**: `archon/ai/background_agent_manager.py`
- **Depends on**: Task 6.1
- **Description**:
  - Rename `qmd_url: str | None = None` → `search_url: str | None = None` in `__init__` and store as `self._search_url`
  - Update pass-through to `ClaudeSession(search_url=self._search_url, ...)`
...
- **Tests (TDD)** — `tests/ai/test_background_agent_manager.py` (update existing):
  - Unit: `test_bam_passes_search_url_to_session` — search_url set → ClaudeSession receives it
...
- **Description**:
  - In `context_provider.py`: rename `startup_context_prompt(self, qmd_enabled: bool = False)` → `startup_context_prompt(self, search_enabled: bool = False)`
  - In `history_compactor.py`:
    - Rename parameter `qmd_enabled` → `search_enabled`
...
- **Tests (TDD)** — `tests/ai/test_history_compactor.py` (update existing):
  - Unit: `test_startup_prompt_search_enabled_mentions_search_tool` — `search_enabled=True` → text contains "search" and "MCP tool"
  - Unit: `test_startup_prompt_search_enabled_no_qmd_text` — `search_enabled=True` → text does not contain "QMD"
  - Unit: `test_startup_prompt_rag_disabled_omits_rag_section` — `search_enabled=False` → no "search" tool mention
  - Checkpoint: `uv run pytest tests/ai/test_history_compactor.py -v`

#### Task 6.4 — Rename `qmd_url → search_url` in `ai/session_manager.py`
- [x] **File**: `archon/ai/session_manager.py`
- **Depends on**: Tasks 6.1, 6.3
- **Description**:
  - Rename `qmd_url: str | None = None` → `search_url: str | None = None` in `__init__` and `self._search_url`
  - Update `_create_session()`: pass `search_url=self._search_url` to `ClaudeSession`
  - Update `startup_context_prompt` call: `self._history_compactor.startup_context_prompt(search_enabled=self._search_url is not None)`
- **Releasable**: after this task, `SessionManager` is fully RAG-aware
- **Tests (TDD)** — `tests/ai/test_session_manager.py` (update existing):
  - Unit: `test_session_manager_passes_search_url_to_session` — `search_url` set → `ClaudeSession` called with it
  - Unit: `test_session_manager_startup_prompt_search_enabled` — `search_url` set → `startup_context_prompt(search_enabled=True)` called
...
- **Description**:
  ...
  - In `_run()`: replace `cfg.qmd` block with:
    ...
    if cfg.search.enabled:
        server_ok = await _ensure_rag_server(cfg.search.host, cfg.search.port)
        if server_ok:
            search_url = f"http://{cfg.search.host}:{cfg.search.port}/mcp"
            logger.info("RAG MCP endpoint: %s", search_url)
        else:
            logger.warning("RAG server unreachable — RAG integration disabled for this session")
    ```
  - Pass `search_url` to `SessionManager(search_url=search_url, ...)` and `BackgroundAgentManager(search_url=search_url, ...)`
...
- **Tests (TDD)** — `tests/gateway/test_gateway.py` (update existing):
  ...
  - Unit: `test_gateway_run_passes_search_url_to_session_manager` — cfg.search.enabled=True, mocked probe → SessionManager called with search_url
  - Unit: `test_gateway_run_rag_disabled_passes_none` — cfg.search.enabled=False → search_url=None
  - Unit: `test_no_qmd_references_in_gateway` — grep the module for "qmd" → zero matches (checked via import inspection)
  - Integration: `test_gateway_rag_lifecycle` — gateway._run() with search.enabled=True, mocked TCP probe failure → search_url=None passed to SessionManager; next call with probe success → search_url set correctly
  - Integration: `test_gateway_rag_full_chain` — gateway config with `search.enabled=True`, mocked TCP probe success → SessionManager called with `search_url` → ClaudeSession built with `mcp_servers["search"]` set → startup prompt contains "search" (verifies config → gateway → session → MCP registration → prompt chain)
  - Checkpoint: `uv run pytest tests/gateway/ -v`

#### Task 6.6 — macOS RAG service in `archon/platform/macos/search_service.py`
- [x] **File**: `archon/platform/macos/search_service.py`
...
- **Description**:
  - `class LaunchdSearchService(PlatformService)`:
    ...
    - `_plist_path` property → `Path.home() / "Library/LaunchAgents/com.archon.search.plist"`
    - `register(dry_run=False) -> int` — writes plist file:
      ```xml
      <?xml version="1.0" encoding="UTF-8"?>
      <!DOCTYPE plist PUBLIC ...>
      <plist version="1.0"><dict>
        <key>Label</key><string>com.archon.search</string>
        <key>ProgramArguments</key>
        <array><string>{sys.executable}</string><string>-m</string><string>archon.search.server</string></array>
      ...
      ```
      where `log_path = Path.home() / ".archon/search/archon-search.log"`. No-op if `dry_run`. Returns 0 on success, 1 on failure.
    - `unregister(dry_run=False) -> int` — removes plist file; no-op if not present
    - `start(dry_run=False) -> int` — `launchctl load <plist_path> && launchctl start com.archon.search`; no-op if `dry_run`
    - `stop(dry_run=False) -> int` — calls `launchctl unload <plist_path>` directly (this both stops the process and unloads the job definition, preventing KeepAlive restart); no-op if `dry_run`. No need to call `launchctl stop` first — `unload` is atomic. **Note**: Using `launchctl stop com.archon.search && launchctl unload <plist_path>` is incorrect: if `launchctl stop` fails (e.g., process already stopped), the `&&` chain prevents `unload` from running, leaving the plist loaded and KeepAlive=true causing an immediate restart. Matches the existing `LaunchdService.stop()` pattern in `archon/platform/macos/service.py`.
    - `restart(dry_run=False) -> int` — stop then start
    - `status() -> ServiceInfo` — `launchctl list com.archon.search`; returns `ServiceInfo`
    - `is_installed() -> bool` — `_plist_path.exists()`
    - `remediation_hint() -> str` — returns "Run `archon search install` to register the RAG service"
    - `pre_activate_cleanup(dry_run=False) -> int` — no-op for RAG service; returns 0
  - Add `get_search_service() -> PlatformService` singleton to `archon/platform/__init__.py`:
    - Same lazy-singleton pattern as `get_service()`: `_search_service: PlatformService | None = None`
    - Update `override(service=None, runtime=None, search_service=None)` to also set `_search_service` when provided
    - Update `reset()` to also clear `_search_service = None`
    - Add `archon/platform/windows/search_service.py` — `WindowsSearchService` stub: all methods print "RAG service management not supported on Windows; run `python -m archon.search.server` manually" and return 1. Wire in `get_search_service()`: detect Windows via `_detect()` (the existing platform detection function) and return `WindowsSearchService()`.
- **Releasable**: after this task, `get_search_service()` is callable on macOS; tests can inject mock via `override(search_service=mock)`
- **Tests (TDD)** — `tests/platform/macos/test_search_service.py`:
  - Unit: `test_search_service_plist_contains_label` — `register()` writes plist with `com.archon.search`
  - Unit: `test_search_service_plist_contains_python_executable` — `sys.executable` in plist
  - Unit: `test_search_service_register_dry_run_no_file` — `dry_run=True` → no file written
  - Unit: `test_search_service_is_installed_true_when_plist_exists` — plist file exists → True
  - Unit: `test_get_search_service_override_in_tests` — `override(search_service=mock); get_search_service()` returns mock; `reset()` clears it
  - Unit: `test_reset_clears_search_service_singleton` — after `override(search_service=mock)`, call `reset()`, then `get_search_service()` returns the platform-appropriate service instance (not the mock)
  - Unit: `test_get_search_service_windows_returns_stub` — on Windows platform (mock `_detect()` to return Windows), `get_search_service()` returns a `WindowsSearchService` instance
  - Checkpoint: `uv run pytest tests/platform/macos/test_search_service.py -v`

#### Task 6.7 — Linux RAG service in `archon/platform/linux/search_service.py`
- [x] **File**: `archon/platform/linux/search_service.py`
- **Depends on**: Task 6.6 (same pattern; `get_search_service()` already added)
- **Description**:
  - `class SystemdSearchService(PlatformService)`:
    - `service_name` property → `"systemd-rag"`
    - `_unit_path` property → `Path.home() / ".config/systemd/user/archon-search.service"`
    ...
    - `start(dry_run=False) -> int` — `systemctl --user start archon-search`
    - `stop(dry_run=False) -> int` — `systemctl --user stop archon-search`
    - `restart(dry_run=False) -> int` — `systemctl --user restart archon-search`
    - `status() -> ServiceInfo` — `systemctl --user status archon-search`
    - `is_installed() -> bool` — `_unit_path.exists()`
    - `remediation_hint() -> str` — returns "Run `archon search install` to register the RAG service"
    - `pre_activate_cleanup(dry_run=False) -> int` — no-op; returns 0
  - The `get_search_service()` singleton (added in Task 6.6) returns `SystemdSearchService()` on Linux
- **Releasable**: after this task, `get_search_service()` is callable on Linux
- **Tests (TDD)** — `tests/platform/linux/test_search_service.py`:
  - Unit: `test_linux_search_service_unit_file_contains_service_name` — `register()` writes unit with `archon-search`
  - Unit: `test_linux_search_service_register_dry_run_no_file` — `dry_run=True` → no file written
  - Checkpoint: `uv run pytest tests/platform/linux/test_search_service.py -v`
...
- **Description**:
  - `archon/ai/pipeline.py`: rename `qmd_url` parameter in constructor → `search_url`; rename `self._qmd_url` → `self._search_url`; update all pass-through calls to `Classifier` and `Decomposer` to use `search_url=`
  - `archon/ai/classifier.py`: rename `qmd_url` in `__init__` → `search_url`; rename `self._qmd_url` → `self._search_url`
  - `archon/ai/decomposer.py`: rename `qmd_url` in constructor → `search_url`; rename `qmd_enabled` → `search_enabled` in the `startup_context_prompt` call (and any internal flag). **Critical**: change `startup_context_prompt(qmd_enabled=False)` → `startup_context_prompt(search_enabled=self._search_url is not None)` — the hardcoded `False` means RAG tool hints never appear in decomposer sub-sessions even when RAG is enabled. Using `self._search_url is not None` ensures decomposer sub-sessions receive the RAG search tool hint when RAG is enabled.
...
- **Tests (TDD)**:
  - Unit: add to `tests/ai/test_decomposer.py` — `test_decomposer_has_no_qmd_url_attribute`: assert `Decomposer` instance has no `_qmd_url` attribute and accepts `search_url` parameter
  - Unit: add to `tests/ai/test_decomposer.py` — `test_decomposer_startup_prompt_search_enabled`: `Decomposer` instantiated with `search_url` set → `startup_context_prompt` called with `search_enabled=True`
  - Unit: add to `tests/ai/test_classifier.py` — `test_classifier_has_no_qmd_url_attribute`: assert `Classifier` instance has no `_qmd_url` attribute and accepts `search_url` parameter
  - Unit: add to `tests/ai/test_pipeline.py` — `test_pipeline_has_no_qmd_url_attribute`: assert `Pipeline` instance has no `_qmd_url` attribute and accepts `search_url` parameter
...
- [x] **Files to update in-place** (incidental `qmd` references — update to use `search_url`/`search_enabled`):
  - `tests/test_installer_py.py` — replace all `qmd_url`, `_prompt_qmd`, `_set_qmd_enabled` references with their `rag` equivalents
  - `tests/ai/test_context_provider.py` — replace `qmd_enabled` parameter with `search_enabled`
  - `tests/config/test_config_example_sync.py` — replace `qmd` section references with `rag`
  - ~~`tests/config/test_loader.py`~~ — **NOT in this list**. This file was already updated in Task 1.2's commit (same commit as the `QmdConfig` → `SearchConfig` refactor). Do not touch it here.
...
- **Description**:
  ...
  - Update 4 files with incidental `qmd` references in-place to use `search_url`/`search_enabled`
...
### Phase 7 — Installer & CLI
> **Releasable**: after Task 7.2 — `archon search install` completes on macOS, registers the service, and Archon can immediately connect to the RAG server.

#### Task 7.1 — `SearchInstaller` class in `archon/search/install.py`
...
- **Description**:
  - `class SearchInstaller`:
    - `__init__(self, config_file: str = "config.toml", dry_run: bool = False) -> None`
      — loads `SearchConfig` from config file; sets `self.cfg`, `self.dry_run`
    ...
    - `def configure_providers(self) -> None` — if `detect_gpu()` returns `True`, writes `providers = ["CUDAExecutionProvider"]` to the `[search]` section of `config.toml` via `tomlkit`. No-op if already set, no-op if `dry_run`.
    - `def create_data_dir(self) -> None` — `Path(self.cfg.db_path).expanduser().mkdir(parents=True, exist_ok=True)`; no-op if `dry_run`
    - **Platform abstraction**: Per the project constraint ("no `platform.system()` / `sys.platform` checks in `archon/`"), `SearchInstaller` must NOT branch on platform directly. Instead, `archon/platform/macos/search_service.py` and `archon/platform/linux/search_service.py` provide `RagPlatformService(PlatformService)` subclasses (analogous to the existing Archon service implementations). A new `get_search_service() -> PlatformService` singleton in `archon/platform/__init__.py` returns the appropriate subclass. `SearchInstaller.write_service_file()` delegates to `get_search_service().register(dry_run=self.dry_run)` and `SearchInstaller.load_service()` delegates to `get_search_service().start(dry_run=self.dry_run)`.
    - (Platform service files `archon/platform/{macos,linux,windows}/search_service.py` are created in Tasks 6.6 and 6.7 — do not recreate them here.)
    - These subclasses override `service_name`, `register()` (writes plist/unit file), `unregister()`, `start()`, `stop()`, `status()` using the same launchctl/systemctl commands but for label `com.archon.search` / service `archon-search`.
    - `def write_service_file(self) -> None` — calls `get_search_service().register(dry_run=self.dry_run)`. The platform-specific plist/unit file content is defined in `archon/platform/macos/search_service.py` (Task 6.6) and `archon/platform/linux/search_service.py` (Task 6.7).
    - `def load_service(self) -> int` — calls `get_search_service().start(dry_run=self.dry_run)`; returns exit code
    - `def unload_service(self) -> int` — calls `get_search_service().stop(dry_run=self.dry_run)`
    - `async def create_history_collection(self) -> None` — instantiates `SearchPipeline` directly via `create_pipeline(self.cfg)` factory, calls `await pipeline.store.connect()`, then calls `await pipeline.ingest_directory(history_dir, history_collection)` inside a `try/finally` block that calls `await pipeline.store.disconnect()` on exit. Does NOT call the running server via HTTP. `run()` calls this via `asyncio.run(self.create_history_collection())`. **Important**: Direct pipeline access bypasses the running server. If the RAG server is running, do not call this simultaneously — LanceDB's local file format does not safely support concurrent writers from separate processes. `run()` **must check `get_search_service().status()` before calling `create_history_collection()`**: if the RAG service is running, print an error message ("RAG server is running — stop it first with `archon search stop` before installing"), and return 1 (failure) without proceeding. This is consistent with `_run_ingest()` which also refuses to write when the service is running. **Note**: This check-then-act pattern has an inherent TOCTOU (time-of-check-to-time-of-use) race condition — the service could start between the check and the write. For v1 this is accepted; users are expected not to start the service manually during install. A future version could add a file lock (e.g., via `fcntl.flock()`) for stronger protection.
    - `def run(self, non_interactive: bool = False) -> int` — full install flow:
      ...
      11. Print "RAG server ready. Enable in config.toml: [search] enabled = true"
    ...
    - `def run_uninstall(self, delete_db: bool = False) -> int`:
      1. `unload_service()`
      2. Call `get_search_service().unregister(dry_run=False)` (removes the plist/unit file via the platform service abstraction).
      3. If `delete_db`: remove `cfg.db_path` directory
      4. Print instructions to remove `[search]` from config
- **Releasable**: after this task, `SearchInstaller` is callable programmatically and from tests
- **Tests (TDD)** — `tests/search/test_install.py`:
  - **(Setup)**: Use `override(search_service=mock_service)` in test setup and `reset()` in teardown for all tests that call `get_search_service()` indirectly. Alternatively, add this as a conftest fixture for `tests/search/` to ensure no test triggers real `launchctl`/`systemctl` calls in CI.
  ...
  - Unit: `test_write_service_file_delegates_to_platform` — `get_search_service().register()` called
  - Unit: `test_write_service_file_dry_run` — dry_run=True → `register(dry_run=True)` called
  - Unit: `test_search_platform_service_macos_plist_contains_label` (`tests/platform/macos/test_search_service.py`) — plist XML contains `com.archon.search`
  - Unit: `test_search_platform_service_macos_plist_contains_python_executable` — `sys.executable` in plist
  ...
  - Unit: `test_installer_run_aborts_when_service_running` — mock `get_search_service().status()` to return running state; verify an error message is printed to stdout and `run()` returns 1; verify `create_history_collection` (and therefore `ingest_directory`) is NOT called
  ...
  - Unit: `test_run_uninstall_stops_and_unregisters_service` — mock `get_search_service()`; verify both `stop()` and `unregister()` are called
...
#### Task 7.2 — `archon search` CLI subcommand in `archon/cli/search_cmd.py` + `main.py`
...
- **Description**:
  - `archon/cli/search_cmd.py`:
    ...
    - `def _run_install(args: argparse.Namespace) -> int`:
      — `SearchInstaller(dry_run=args.dry_run).run(non_interactive=args.non_interactive)`
    - `def _run_uninstall(args: argparse.Namespace) -> int`:
      — `SearchInstaller().run_uninstall(delete_db=args.delete_db)`
    - `def _run_start(args: argparse.Namespace) -> int`:
      — calls `get_search_service().start()`; prints status
    - `def _run_stop(args: argparse.Namespace) -> int`:
      — calls `get_search_service().stop()`
    - `def _run_status(args: argparse.Namespace) -> int`:
      — calls `get_search_service().status()` and prints state; then fetches collection stats by building only a `SearchStore(cfg.search.db_path)`, calling `asyncio.run(async_wrapper())` where `async_wrapper` does `connect -> list_collections -> disconnect` wrapped in try/finally. No embedder, reranker, chunker, or parser needed — `SearchStore` is the only dependency for listing. LanceDB cross-process concurrent access is not guaranteed safe. The try/except around `list_collections()` is mandatory (not optional) — always catch LanceDB errors and display "Stats unavailable — server may be writing" rather than crashing. If the RAG service is not running or an error occurs, prints "unreachable" and returns non-zero exit code.
    - `def _run_ingest(args: argparse.Namespace) -> int`:
      — loads config; determines `path = args.path or cfg history_dir`, `collection = args.collection or cfg.search.history_collection`. Before running: check if the RAG service is running via `get_search_service().status()`. If running AND the service is reachable, warn the user and exit with a non-zero return code (do not proceed with direct write). Proceed with direct pipeline only when the service is confirmed stopped. This prevents data corruption from concurrent LanceDB writers. **Note**: This check-then-act pattern has an inherent TOCTOU (time-of-check-to-time-of-use) race condition — the service could start between the check and the write. For v1 this is accepted; users are expected not to start the service manually during an active ingest. A future version could add a file lock (e.g., via `fcntl.flock()`) for stronger protection. When proceeding: builds pipeline via `create_pipeline(cfg)` and wraps the full async flow — creates an async function that calls `await pipeline.store.connect()`, then `await pipeline.ingest_directory(path, collection)`, then `await pipeline.store.disconnect()` in a `finally` block — and calls `asyncio.run()` on that wrapper function. Do NOT call `asyncio.run(pipeline.ingest_directory(...))` directly — store must be connected first or every store operation raises `RuntimeError("SearchStore not connected")`. Does NOT call the running server via HTTP — consistent with installer's direct pipeline approach, requires no MCP client library.
  - `archon/cli/main.py` additions:
    ...
    - Add dispatch: `if args.command == "search": from archon.cli.search_cmd import run_search; return run_search(args)`
- **Releasable**: after this task, all `archon search` subcommands are available from the CLI
- **Tests (TDD)** — `tests/cli/test_search_cmd.py`:
  - **(Setup)**: Use `override(search_service=mock_service)` in test setup and `reset()` in teardown for all tests that call `get_search_service()` indirectly. Alternatively, add this as a conftest fixture for `tests/cli/` to ensure no test triggers real `launchctl`/`systemctl` calls in CI.
  - Unit: `test_search_install_delegates_to_installer` (mocked SearchInstaller) — `run()` called
  - Unit: `test_search_install_dry_run_flag` — `--dry-run` → `dry_run=True` passed
  - Unit: `test_search_uninstall_delegates` (mocked SearchInstaller) — `run_uninstall()` called
  - Unit: `test_search_ingest_no_args_uses_history_dir` (mocked pipeline) — history dir used as path
  - Unit: `test_search_ingest_with_path_and_collection` — custom path + `--collection` forwarded correctly
  - Unit: `test_main_rag_command_registered` — `archon search --help` exits 0 and includes "rag" subcommands
  - Unit: `test_search_start_calls_platform_service` — `archon search start` → `get_search_service().start()` called
  - Unit: `test_search_stop_calls_platform_service` — `archon search stop` → `get_search_service().stop()` called
  - Unit: `test_search_status_prints_service_state` — `archon search status` → output contains "running" or "stopped"
  - Unit: `test_search_status_server_unreachable_prints_warning` — server not reachable → output contains "unreachable", returns non-zero exit code
  - Unit: `test_search_ingest_aborts_when_service_running` — mock `get_search_service().status()` to return running state; verify an error message is printed to stdout, the command returns non-zero exit code, and `ingest_directory` is NOT called (no write attempted)
  - Unit: `test_search_status_disconnects_on_list_collections_failure` — mock `list_collections()` to raise; verify `store.disconnect()` is still called AND the output contains "Stats unavailable"
  - Unit: `test_search_status_shows_unavailable_on_lock_error` — mock `list_collections()` to raise a LanceDB error; verify "Stats unavailable" appears in stdout output (not just that disconnect was called)
  - Unit: `test_search_ingest_disconnects_on_failure` — mock `ingest_directory` to raise; verify `store.disconnect()` is called
  - Checkpoint: `uv run pytest tests/cli/test_search_cmd.py -v`
  Final Phase 7 coverage gate: `uv run pytest tests/search/ tests/cli/test_search_cmd.py tests/platform/macos/test_search_service.py tests/platform/linux/test_search_service.py --cov=archon/search --cov-fail-under=85 --cov-report=term-missing`

---

### Phase 8 — Documentation
> **Releasable**: after Task 8.2 — all user-facing docs reflect the new `[search]` config and `archon search` CLI.
...
- **Description**:
  ...
  - Add a `[search]` section with all fields, defaults, and inline comments explaining each option, model choices, and the ~150 MB model download warning for first install (matching the Known Limitations section)
...
- **Description**:
  - `user_manual.md`: add "RAG Search" section after the existing "QMD" section (or replace if QMD section exists):
    - Installation steps (`archon search install`)
    - Hardware requirements (~2 GB RAM recommended for running models; ~150 MB download on first install)
    - How to add collections (`archon search ingest /path --collection name` or via MCP `ingest_directory` tool)
  ...
  - `CLAUDE.md`: update `[search]` config fields table (currently documents `[qmd]`); update `archon/ai/context_provider.py` entry to reflect `search_enabled` parameter name
  ...
  - `Documentation/Architecture/120_services_and_integration_architecture.md`: replace `mcp_servers["qmd"]` with `mcp_servers["search"]`; update section "4. QMD MCP (Optional)" → "4. RAG MCP (Optional)"; update all Mermaid nodes referencing QMD
  ...
  - `Documentation/ADRs/09_qmd_compatible_history_format.md`: archive (move to `Documentation/Completed/`) and create `Documentation/ADRs/09_search_history_format.md` as its replacement describing the new RAG integration and technology selection rationale (reference `Documentation/Completed/26_search_integration_research.md`)
  - `CLAUDE.md`: update remaining `[qmd]` references in the config section that are not already handled by Task 1.2 (e.g., config table row for `[qmd] history_collection` → `[search] history_collection`)
  - Delete `Documentation/Backlog/RAG integration for multi-format document search.md` (already archived to `Documentation/Completed/26_search_integration_research.md`)
- **Releasable**: N/A (docs-only)
- **Tests (TDD)**: N/A
