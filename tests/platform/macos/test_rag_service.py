...
def test_rag_service_service_name() -> None:
    """service_name property returns 'launchd-rag'."""
    from archon.platform.macos.rag_service import LaunchdRagService

    svc = LaunchdRagService()
    assert svc.service_name == "launchd-rag"
...
def test_rag_service_remediation_hint() -> None:
    ...
    assert "archon rag install" in hint
...
def test_rag_service_start_not_installed_returns_one(tmp_path: Path) -> None:
    ...
    from archon.platform.macos.rag_service import LaunchdRagService
    svc = LaunchdRagService()
    plist = tmp_path / "com.archon.rag.plist"  # does not exist
    with patch.object(type(svc), "_plist_path", new_callable=lambda: property(lambda self: plist)):
        assert svc.start() == 1
