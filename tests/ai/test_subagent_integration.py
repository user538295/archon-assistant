...
# ──────────────────────────────────────────────────────────────────
# Bug.003 — agent lifecycle always shown (no quiet mode override)
# ──────────────────────────────────────────────────────────────────
...
def test_load_config_quiet_mode_migrated_to_normal(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Legacy config with mode='quiet' is silently migrated to 'normal' on load."""
...
def test_load_config_notifications_agents_section_silently_ignored(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Legacy [notifications.agents] section in config is silently ignored (no error)."""
    monkeypatch.delenv("TELEGRAM_BOT_TOKEN", raising=False)
    env = tmp_path / ".env"
    env.write_text("TELEGRAM_BOT_TOKEN=test\n")
    cfg_file = tmp_path / "config.toml"
    ...
    # Must not raise — legacy section is ignored
    cfg = load_config(env_file=env, config_file=cfg_file)
    assert cfg.notifications.mode == "normal"
