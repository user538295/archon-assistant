...
async def test_handle_message_no_history_injection_when_no_files() -> None:
    """No history notice when no files were injected."""
    mgr = _mock_session_manager(Response(content="Hi"))
    msg = _mock_message("hello")
    notif = NotificationsConfig(mode="debug")
...
def test_rag_injection_visible_in_verbose() -> None:
    """ContextInjectedEvent with injection_type='search_retrieval' renders as 🔍 Search format in verbose mode."""
    notif = NotificationsConfig(mode="verbose", interval_minutes=0)
    ...
    result = format_event(event, _split, notifications=notif)
    assert result == ["🔍 Search: 5 chunks from col1, col2"]


def test_rag_injection_visible_in_debug() -> None:
    """ContextInjectedEvent with injection_type='search_retrieval' renders as 🔍 Search format in debug mode."""
    notif = NotificationsConfig(mode="debug", interval_minutes=0)
    event = ContextInjectedEvent(
        injection_type="search_retrieval",
        size_chars=500,
    ...
    result = format_event(event, _split, notifications=notif)
    assert result == ["🔍 Search: 3 chunks from docs, notes"]


def test_rag_injection_silent_in_quiet() -> None:
    """ContextInjectedEvent with injection_type='search_retrieval' is suppressed in quiet mode."""
    notif = NotificationsConfig(mode="quiet", interval_minutes=0)
    event = ContextInjectedEvent(
        injection_type="search_retrieval",
        size_chars=500,
...
def test_rag_injection_silent_in_normal() -> None:
    """ContextInjectedEvent with injection_type='search_retrieval' is suppressed in normal mode."""
    notif = NotificationsConfig(mode="normal", interval_minutes=0)
    event = ContextInjectedEvent(
        injection_type="search_retrieval",
        size_chars=500,
...
def test_rag_injection_no_detail_falls_through_to_generic_format() -> None:
    """search_retrieval with detail=None falls through to generic 📌 Context injected format."""
    notif = NotificationsConfig(mode="verbose", interval_minutes=0)
    event = ContextInjectedEvent(
        injection_type="search_retrieval",
        size_chars=100,
    ...
    assert "search_retrieval" in result[0]


def test_rag_injection_html_escapes_detail() -> None:
    """HTML-special characters in the search detail string are escaped."""
    notif = NotificationsConfig(mode="verbose", interval_minutes=0)
    event = ContextInjectedEvent(
        injection_type="search_retrieval",
        size_chars=500,
