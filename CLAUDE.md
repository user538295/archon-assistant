...
- `search_client.py`: `SearchClient` — async HTTP client adapter for the `archon-search` REST API (sole import boundary); `get_search_client()` / `reset_search_client()` singletons; all methods return `None`/`[]`/status code on failure, never raise; HTTP read-back methods: `telemetry_stats()` (`GET /telemetry/stats`), `telemetry_entries()` (`GET /telemetry/entries`); `search(collection, query, top_k)` — `POST /search`, returns `list[dict]` or `[]` on failure. `SearchApiKeyAuth(httpx.Auth)` — lazy key load (env var `ARCHON_SEARCH_API_KEY` first, then `~/.archon/.search.env`), caches on success, clears cache and retries on 401, logs ERROR only when retry with valid key also fails; WARNING when no key found; key value never appears in logs; wired into `SearchClient` by default
