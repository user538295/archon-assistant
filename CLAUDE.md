...
- `search_client.py`: `SearchClient` — async HTTP client adapter for the `archon-search` REST API (sole import boundary); `get_search_client()` / `reset_search_client()` singletons; all methods return `None`/`[]`/status code on failure, never raise
