...
- `[search] enabled` (default `true` — search is on by default since FEAT-046), `url` (default `"http://127.0.0.1:8765"`), `max_parallel_collections` (max concurrent search operations; default `3`), `top_k_return` (results returned after reranking; default `5`). **`archon-search` is an external PyPI package** (`https://pypi.org/project/archon-search/`, dev repo `git@github.com:user538295/archon-search.git`) — installed as a transitive dependency of Archon. Auth key lives in `~/.archon/.search.env` (not config.toml); override with `ARCHON_SEARCH_API_KEY` env var. Server-side fields (`db_path`, `embedding_model`, `chunk_size`, `pinned_collections`, `routing_*`, `watch`, etc.) are configured in `~/.archon/archon-search.toml` and emit a deprecation warning if present in `config.toml`. Archon's `_run_archon_search()` (`archon/cli/search_cmd.py`) injects `ARCHON_SEARCH_CONFIG=~/.archon/archon-search.toml` + `ARCHON_SEARCH_KEY_FILE=~/.archon/.search.env` env vars when spawning the `archon-search` subprocess, so the standalone package (whose own defaults point at `~/.archon-search/`) reads from Archon's legacy paths in this deployment. `[telemetry]` in `archon-search.toml`: `enabled` (default `false`), `retention_days` (default `30`), `log_dir` (default `"~/.archon/search-logs"`) — opt-in local JSONL query logging; `export_enabled = true` logs a WARNING and is ignored (reserved for FEAT-039d); HTTP read-back endpoints: `GET /telemetry/stats` (aggregated query statistics), `GET /telemetry/entries` (raw JSONL log entries with optional filters)
