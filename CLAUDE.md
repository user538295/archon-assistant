...
- `[search] enabled`, `url` (default `"http://127.0.0.1:8765"`), `max_parallel_collections` (max concurrent search operations; default `3`), `top_k_return` (results returned after reranking; default `5`) — server-side fields (`db_path`, `embedding_model`, `chunk_size`, `pinned_collections`, `routing_*`, `watch`, etc.) are configured in `archon-search.toml` and emit a deprecation warning if present in `config.toml`
