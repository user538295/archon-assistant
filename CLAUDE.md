...
- `progress.py` (`archon_search/progress.py` in `packages/archon-search`): `IndexingStateStore` — atomic read/write of `.indexing_state.json`; `CollectionProgress` + `IndexingState` dataclasses for per-collection indexing state tracking
